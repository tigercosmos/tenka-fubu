...
| 迷你地圖 | 1684, 844 | 224×224 | 勢力色塊＋視野框（正方，邊長＝12 §3.1.8 `UI.minimapSizePx`＝224）；點擊跳轉鏡頭（04） |
...
#### 3.8.1 調略畫面（PlotPanel）

全螢幕 panel 1440×880（同外交，背景 60% 墨黑 scrim，時間不暫停）。上半「進行中調略」
列表、下半「新調略精靈」。08 §6.1 所稱之調略畫面即本 panel（勘誤 E-66）。

```
┌ 調略 ─────────────────────────────────────────────［✕］┐
│ ── 進行中調略（3）─────────────────────────────────────  │
│ 種別   目標           執行武將     進度        剩餘      │
│ 引拔  今川・岡部元信   瀧川一益   ██████░░ 62%   8日     │ ← 列點擊＝選取，可［中止］
│ 流言  今川（家中）     竹中半兵衛 ███░░░░░ 31%  14日     │
│ 內應  鳴海城・城主     蜂須賀正勝 ████████ 就緒  ─      │ ← 就緒者顯示［發動內應］
│ ── 新調略精靈 ─────────────────────────────────────────  │
│ ① 種別：(○)引拔  ( )流言  ( )內應                        │
...
#### 3.8.2 外交來使 modal（EnvoyModal）

他勢力向我方提案時彈出（modal，時間暫停）；對應 08 §6.1 之來使 modal（勘誤 E-67）。
置中 720×520。
...
#### 3.11.3 捕虜處置 modal（CaptiveModal）

落城／野戰俘獲敵將時彈出（modal，時間暫停；§3.14.3 步驟 9 引用）。置中 640×560，
逐一處置本次俘獲之武將。對應 ModalId `captive`（勘誤 E-77）。
...
│ 知略 █████▊    58    政務 ████▍     44       │
│ 身分：侍大將    忠誠（對舊主）88             │
│ 特性：[城將][寡默]                           │ ← chip 懸停顯效果（06）
│ ── 處置（三選一）─────────────────────────  │
│ ［招降］ 成功率約 24%（對舊主忠誠越高越難，06）│ ← 成功則入我家臣（初始忠誠依 06）
│ ［釋放］ 感情/威信影響見 08                   │
│ ［處斬］ 威信＋…；對舊主/親族之影響（06）     │
│  ● ○     ← 多名俘虜時逐一處置指示點           │
└──────────────────────────────────────────────┘
...
- 招降成功率、釋放與處斬之威信/感情/忠誠影響全依 `plan/06` §3.7；三選一即發
  `CmdHandleCaptive`（`CaptiveAction`＝`'recruit'|'release'|'execute'`，02／06）。
  多名俘虜逐一處置，全部完成後 modal 自動關閉（勘誤 E-77）。
...
#### 3.15.1 原創視覺策略與參考圖萃取規則

視覺定位為「**低飽和戰國沙盤 × 現代戰略資訊層**」。玩家進入主畫面後，第一眼應讀到地形骨架，
第二眼讀到道路／城池／領域，第三眼才是軍隊／命令／警告；HUD 退居畫面邊緣，詳細資訊只在 hover、
選取、分析圖層或情境面板中展開。歷史感來自和紙、墨、木、漆、印章與少量金箔的原創材質語彙，
不靠仿製任何商業遊戲的面板、icon、城模或貼圖。

研究《信長之野望・新生》公開畫面時，只萃取以下抽象問題：

1. 全國／地方／近景三種縮放下，哪些地形、道路、城池、軍隊與標籤仍被保留。
2. 行軍時如何同時呈現路線、方向、兵力、補給、到達時間與風險，又不遮蔽地圖。
3. 城池如何兼作地標、選取目標與資訊錨點；戰鬥／攻城時哪些警告必須留在場景內。
4. 常駐 HUD 與情境面板如何分工，以及過密、小字、路點難點選、單位重疊等問題如何避免。

Reference board 每張圖必填來源 URL、存取日期、抽象觀察、適用情境與「不可直接沿用」；禁止描摹原作
版面比例、面板位置、icon 輪廓、肖像、家紋、字型、城模、貼圖、配色數值與動效。最終 wireframe、
icon grid、資產與動效必須從本作 Art Bible 重新設計。官方研究入口由 18 §3.9.1 統一維護。

主畫面視覺驗收：在 1280×720 不開面板時，主要戰場不可被常駐 HUD 切碎；overview／operational／close
三段截圖皆能辨認主要道路、城池、軍隊與選取物；任一狀態不得只靠紅／綠色差表達；小型路點／icon
須有至少 32 CSS px 的隱形 hit area。字級、對比、標籤避讓與 safe area 以 12／17 的 gate 為準。

#### 3.15.2 色票（建議 hex，12 定案）
...
export type PanelId =
  ...
  | 'plot'          // 調略（進行中列表＋新調略精靈；params.mode?: 'list'|'new'、params.kind?、params.targetId?）
  | 'policy'        // 政策
  | 'corps'         // 軍團
  | 'taimei'        // 大命
  | 'reports'       // 報告中心
...
- [ ] **T16　M6-V 主畫面 visual slice**（§3.3／§3.15）：將既有 ResourceBar、SpeedControl、MiniMap、
      IconButton 與 ContextPanel 組裝至 MainScreen；通知改可收合；完成 Art Bible／reference board 與
      1280×720 safe area。
      驗收：主畫面不再以 emoji、Unicode 圖案、文字按鈕與 inline style 作主要導覽；三段地圖 LOD 下
      HUD 都不遮主要戰場；reviewer 3 秒內可指出敵我、城池歸屬、主要行軍方向與選取物。
...
- **D21　M6-V 以「原創戰國沙盤＋現代資訊層」取代單純色票定調（2026-07-14）**：既有 §3.15 只有
  色票、紙紋與字體，無法裁定地圖／HUD 的閱讀優先序，也無法防止 reference 直接變成仿製。新增
  §3.15.1 明確定義地形→道路／城池／領域→軍隊／命令／警告的層級、reference board 欄位與禁抄清單；
  18 的 M6-V 為施工與整合驗收單一真相，04／12／17 分別擁有圖層、元件／素材與測試細節。

- **D21（2026-07-22・[M6-V9] HUD 組裝落地裁決，交叉引用 plan/12）**：MainScreen 依 §3.3 組裝既有
  元件——頂墨帶 48px（`ResourceBar`：金/糧皆顯示每月「淨額」、兵力＝駐城兵合計/容量合計＋
  `SpeedControl`＋☰ disabled 占位，系統選單本體 M8-20）；左欄家紋塊＋四域 56×56 `IconButton`
  （外交/軍團/大命鈕俟 M6/M7 功能落地再增列）；右下 `MiniMap`（點擊→`panToWorld` 鏡頭補間；
  底部快覽條開啟時垂直讓位——§3.3 之 1920 佈局面板寬 1536 與小地圖不相疊，1280 窄版面以抬高
  等價落實）；底部 `ContextPanel` 城/郡/軍三態快覽條。**點擊語意**：點地圖城/郡僅 `setSelection`
  開快覽條，完整面板一律由快覽條動作鈕或左欄開啟（消除「點城即彈滿版面板」的雙開）。
  未定義 CSS 變數 `--accent-red`/`--danger` 修繕為 vermilion 家族（出陣鈕自此可見）。
  單一真相＝`docs/design/m6-v9-hud-readability.md`。

### 8.1 勘誤消化記錄（19 §3.13）

- **D11（2026-07-07・E-66）調略畫面（PlotPanel）落地**：08 §6.1 宣稱之調略畫面於 11
  原缺對應 Panel／wireframe。依 19 §3.13 E-66（「11 補調略 Panel：進行中列表＋新調略
  精靈」）於 §3.8.1 補 PlotPanel wireframe，§4 `PanelId` 增 `'plot'`（含 `mode/kind/targetId`
  參數）；相關字串由 13 補齊。
- **D12（2026-07-07・E-67）外交來使 modal（EnvoyModal）落地**：依 19 §3.13 E-67（「11 §4
  增外交來使 modal：來使勢力／提案／接受拒絕，婚姻顯示成婚武將」）於 §3.8.2 補來使 modal
  wireframe，§4 `ModalId` 增 `'envoy'`（`clanId/pactKind` 參數）。
- **D13（2026-07-07・E-69）迷你地圖統一為正方**：依 19 §3.13 E-69（世界座標 4096² 正方、
  均勻縮放，迷你地圖本質正方，統一 11 與 12）將 §3.3 迷你地圖由 288×192 改為 224×224
  （＝12 §3.1.8 `UI.minimapSizePx`），位置由 (1620, 876) 調為 (1684, 844) 以維持原右下
  錨定（右緣 1908、下緣 1068 不變）；HUD 尺寸表與 wireframe 同步更新。
- **D14（2026-07-07・E-76）協定圖示措辭修正**：依 19 §3.13 E-76（「修 11 措辭：同盟
  handshake、婚／休／從用文字徽章」），§3.8 協定標示改為同盟用 `handshake` icon、
  婚姻／停戰／從屬用文字徽章（「婚」「休」「從」）；不要求 12 增 icon（12 `IconName`
  協定圖示僅 `handshake`）。
- **D15（2026-07-07・E-77）捕虜處置 modal wireframe 補齊**：ModalId `captive` 原有型別
  （§4）與 §3.14.3 引用但缺 wireframe。依 19 §3.13 E-77（「11 §3.x 補捕虜處置 modal
  wireframe：武將資訊＋招降/釋放/處斬」）於 §3.11.3 補 wireframe，處置經 `CmdHandleCaptive`
  （`CaptiveAction`，02／06）發出。
- **D16（2026-07-10・E-27）朝廷頁「獻金按鈕」改為持續型獻金工作 UI**：02 樞紐定案已刪除
  一次性 `CmdDonateCourt`，朝廷獻金併入 `CmdStartDiploWork`／`CmdStopDiploWork`
  （`target:'court'`），機制（`courtFavor` 累積／衰減）依 08 §3.5。依 19 §3.13 E-27，
