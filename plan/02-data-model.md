...
export interface TransportOrder {
  ...
  edgeProgress: number;        // 當前邊已推進進度（04）
...
export type Command =
  ...
  | CmdDonateCourt | CmdRequestCourtRank | CmdRequestMediation
...
export interface CmdDonateCourt extends CommandBase {       // 朝廷獻金（一次性；持續型獻金工作重寫見勘誤 E-27，v1.0 暫留一次性）
...
> `AiState.intentLog` 為 **transient**（不序列化、不入 golden hash，決策已反映於 Command 紀錄與狀態，
> 09 §8-D5）；`AiClanState` 持久化。各勢力 `AiClanState.persona` 於載入時由 persona 型錄解析：
> `persona ← personas[clan.personaId]`（型錄見 09 §3.2；`AiPersonaId` 定義於 §3.2）。

```ts
// src/core/ai/types.ts —— GameState.ai 分支（09 §4 為行為真相來源）
export interface AiState {
  clans: Record<ClanId, AiClanState>; // 以 clanId 為 key（AI 與玩家勢力皆持有）
  intentLog: AiIntent[];              // 環形緩衝決策紀錄（容量 BAL.aiIntentLogSize；transient，不序列化）
...
export interface AiClanState {
  clanId: ClanId;
  persona: AiPersona;            // 由 personas[clan.personaId] 於載入時解析（勘誤 E-60）
...
- **E-36**：`RoadEdge` 欄位 `kind/length`→`type/grade/baseDays`（§4.7）；依 04 baseDays 模型。
...
- **E-60**：`GameState.ai: AiState` 收錄（§4.1／§4.20）；`intentLog`／`AiIntent` 標 transient 不序列化、`AiClanState` 持久化、`persona ← personas[personaId]`；依 09 §4。
