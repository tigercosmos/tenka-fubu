...
export type BattleId   = string & Brand<'BattleId'>;    // 'battle.000007'
export type SiegeId    = string & Brand<'SiegeId'>;     // 'siege.000003'
export type CorpsId    = string & Brand<'CorpsId'>;     // 'corps.000001'
export type ProposalId = string & Brand<'ProposalId'>;  // 'prop.000118'
...
export type Rank =                                                    // 身分六階（00 §4）
  ...
  | 'karo'             // 家老（可任軍團長）
  | 'shukuro';         // 宿老
export type ArmyStatus =                                              // 部隊狀態機（僅此五態）
  | 'marching'      // 行軍
  | 'engaged'       // 交戰（野戰/合戰中）
  | 'sieging'       // 攻城
  | 'subjugating'   // 制壓（翻轉敵郡）
  | 'returning';    // 歸還
export type BattleMode  = 'auto' | 'tactical';    // 野戰自動解算 / 合戰戰術戰場
...
export type DevelopFocus   = 'agri' | 'commerce' | 'security'; // 直轄郡開發重點：農業/商業/治安（語意見 05）
export type BattleOrderKind = 'advance' | 'hold' | 'charge' | 'withdraw'; // 合戰部隊命令（語意見 07）
...
| reports（上限 `BAL.reportMaxKept` = 600 筆） | 600 | ~230 B | ~138 KB |
...
export interface GameState {
  ...
  battles:   Record<BattleId, Battle>;        // 進行中野戰/合戰
  sieges:    Record<SiegeId, Siege>;          // 進行中攻城戰
  corps:     Record<CorpsId, Corps>;          // 軍團
  diplomacy: DiplomacyState;                  // 外交列＋進行中外交工作
...
export interface Clan {
  ...
  color: string;              // 勢力色 '#rrggbb'（地圖渲染；劇本資料指定）
...
export interface Officer {
  ...
  ldr: number;                  // 統率，整數 1..BAL.abilityMax（120）
  val: number;                  // 武勇，整數 1..120
  int: number;                  // 知略，整數 1..120
  pol: number;                  // 政務，整數 1..120
  traits: TraitId[];            // 特性（被動技），0..BAL.maxTraitsPerOfficer（4）個，不重複
  tactics: TacticId[];          // 合戰戰法，0..2 個（型錄見 07）
  rank: Rank;                   // 身分六階；升格規則見 06
  merit: number;                // 功績，整數 ≥0（累積值，升格門檻見 06）
  loyalty: number;              // 忠誠，整數 0..100；當主恆 100（INV-08）；<30 有出奔/被引拔風險
  isKin: boolean;               // 一門眾（與當主同族）；影響忠誠與繼承（06）
...
export interface Castle {
  ...
  facilities: FacilitySlot[];  // 城下施設 slot；長度固定：main=BAL.facilitySlotsMain（6）、branch=BAL.facilitySlotsBranch（3）
  districtIds: DistrictId[];   // 所轄郡，2..4 個；與 District.castleId 互為鏡像（INV-03）
}

export interface FacilitySlot {
  typeId: FacilityTypeId | null;  // 已建/建設中的施設種類；null=空地
  buildRemainingDays: number;     // 剩餘工期（日），整數 ≥0；0=已完工；>0=建設中（效果未生效）
}
```
...
export interface RoadEdge {
  ...
  kind: RoadKind;              // 'land' 街道 / 'sea' 海路（海路行軍修正見 04）
  length: number;              // 邊長（世界座標距離，預計算浮點數 >0）；行軍日數 = length / 部隊日速（04）
}
...
export interface Army {
  ...
  status: ArmyStatus;          // 狀態機：marching / engaged / sieging / subjugating / returning
  ...
  pathCursor: number;          // 已抵達之 path 索引，整數 0..path.length-1
  posNodeId: MapNodeId;        // 最近抵達節點（= path[pathCursor]）
  edgeProgress: number;        // 往 path[pathCursor+1] 的邊上進度 0..1（浮點）；已達終點為 0
  battleId: BattleId | null;   // status='engaged' 時所屬戰鬥；否則 null（INV-13）
...
### 4.9 Battle（野戰／合戰進行中狀態）

```ts
export interface Battle {
  id: BattleId;
  mode: BattleMode;            // 'auto' 野戰自動解算 / 'tactical' 合戰戰術戰場
  nodeId: MapNodeId;           // 發生地點節點
  attackerArmyIds: ArmyId[];   // 攻方部隊（≥1；後續加入的援軍附加於尾端）
  defenderArmyIds: ArmyId[];   // 守方部隊（≥1）
  startDay: number;            // 開戰絕對日
  rounds: number;              // 已進行回合數，整數 ≥0（auto：1回合=1日；tactical：子迴圈回合）
  tactical: TacticalBattleState | null; // mode='tactical' 時存在；'auto' 為 null
}

/** 合戰戰術戰場（modal 子迴圈中演進；欄位語意與規則見 07） */
export interface TacticalBattleState {
  fieldW: number;              // 戰場格寬（建議 BAL.kassenFieldW = 24）
  fieldH: number;              // 戰場格高（建議 BAL.kassenFieldH = 16）
  units: BattleUnit[];         // 雙方全部隊伍單位
  round: number;               // 合戰內部回合，整數 ≥0；達 BAL.kassenMaxRounds（60）強制結束
}

export interface BattleUnit {
  officerId: OfficerId;        // 帶隊武將（大將或副將）
  side: 'attacker' | 'defender';
  soldiers: number;            // 該單位兵數（人），整數 ≥0
  morale: number;              // 單位士氣 0..100
  x: number; y: number;        // 戰場格座標，整數 0..fieldW-1 / 0..fieldH-1
  order: BattleOrderKind;      // 當前命令：advance/hold/charge/withdraw
  tacticGauge: number;         // 戰法計量 0..100；滿 100 可發動戰法（07）
}
```

合戰結束時由 07 的結算函式計算 `AweLevel`（威風）並發出 `battle.ended` 事件；
威風不存於 Battle，其擴散效果直接改寫周邊 `District.ownerClanId` 與 `Castle.morale`。
...
export interface DiplomacyState {
  ...
  missions: DiploMission[];                // 進行中外交工作（同一 from→to 至多一件，INV-14）
}
...
export interface DiplomacyRow {
  ...
  sentimentAtoB: number;       // a 對 b 的感情，整數 0..100（50=中立；預設 50，劇本可覆寫史實仇誼）
  sentimentBtoA: number;       // b 對 a 的感情，整數 0..100
...
export interface DiploMission {
  ...
  toClanId: ClanId;            // 對象方（INV-14：from ≠ to、皆 alive）
  officerId: OfficerId;        // 執行武將（serving、屬 from 方；執行期間不可出陣，08）
  startDay: number;            // 開始絕對日
  goldPerMonth: number;        // 每月投入金錢（貫），整數 ≥0；信用累積速率參數（08）
}
```
...
export interface CourtState {
  courtFavor: Record<ClanId, number>; // 各勢力朝廷友好度，整數 0..100；獻金累積、敘任消耗（08）
...
// 官位/幕府役職的持有狀態存於 Clan.courtRank / Clan.shogunateTitle（單一真相）。
```
...
export interface ClanPolicyState {
  clanId: ClanId;
  active: PolicyId[];          // 已施行政策（不重複；同時數量上限 BAL.maxActivePolicies = 4，05）
...
// reports 陣列新→舊排列；超過 BAL.reportMaxKept（600）時自尾端捨棄已讀舊報告。
```
...
export type Command =
  // ── 內政（語意見 05）──
  | CmdGrantFief | CmdSetDevelopFocus | CmdBuildFacility | CmdDemolishFacility
  | CmdConscript | CmdTransport | CmdEnactPolicy | CmdRevokePolicy
  | CmdAppointLord | CmdSetCastleControl
  // ── 軍事（語意見 04/07）──
  | CmdMarch | CmdSetArmyTarget | CmdRecallArmy
  | CmdStartKassen | CmdBattleOrder | CmdSetSiegeMode
  // ── 武將（語意見 06）──
  | CmdRecruitRonin | CmdRewardOfficer | CmdHandleCaptive
  // ── 外交與調略（語意見 08）──
  | CmdStartDiploWork | CmdStopDiploWork | CmdProposePact | CmdBreakPact
  | CmdDonateCourt | CmdRequestCourtRank | CmdRequestMediation | CmdStartPlot
  // ── 軍團（語意見 09）──
...
export interface CmdBuildFacility extends CommandBase {     // 建設城下施設
  type: 'buildFacility'; castleId: CastleId;
  slotIndex: number;             // 0-based，須為空 slot
  facilityTypeId: FacilityTypeId;
}
export interface CmdDemolishFacility extends CommandBase {  // 拆除施設（含中止建設）
  type: 'demolishFacility'; castleId: CastleId; slotIndex: number;
}
export interface CmdConscript extends CommandBase {         // 徵兵（人口→城兵力）
  type: 'conscript'; castleId: CastleId;
  soldiers: number;              // 目標徵募數（人），整數 >0；成本與人口扣減見 05
}
export interface CmdTransport extends CommandBase {         // 城際輸送兵力/兵糧
  type: 'transport'; fromCastleId: CastleId; toCastleId: CastleId;
  soldiers: number;              // ≥0
  food: number;                  // ≥0（石）；兩者不得同時為 0
}
...
export interface CmdStartKassen extends CommandBase {       // 對進行中野戰發動合戰（門檻見 07）
  type: 'startKassen'; battleId: BattleId;
}
export interface CmdBattleOrder extends CommandBase {       // 合戰內下令（modal 子迴圈中）
  type: 'battleOrder'; battleId: BattleId; officerId: OfficerId;
  order: BattleOrderKind;
  targetX: number | null; targetY: number | null;           // advance/charge 的目標格；hold/withdraw 為 null
  tacticId: TacticId | null;                                 // 同時發動戰法；null=不發動
}
export interface CmdSetSiegeMode extends CommandBase { type: 'setSiegeMode'; siegeId: SiegeId; mode: SiegeMode; }

// —— 武將 ——
export interface CmdRecruitRonin extends CommandBase { type: 'recruitRonin'; officerId: OfficerId; } // 登用浪人
export interface CmdRewardOfficer extends CommandBase {     // 褒賞（提忠誠）
  type: 'rewardOfficer'; officerId: OfficerId;
  gold: number;                  // 支出金錢（貫），>0；忠誠增量公式見 06
}
export interface CmdHandleCaptive extends CommandBase {     // 捕虜處置
  type: 'handleCaptive'; officerId: OfficerId; action: CaptiveAction;
}

// —— 外交與調略 ——
export interface CmdStartDiploWork extends CommandBase {    // 開始外交工作（派使者累積信用）
  type: 'startDiploWork'; targetClanId: ClanId; officerId: OfficerId;
  goldPerMonth: number;          // 每月投入（貫），≥0
}
export interface CmdStopDiploWork extends CommandBase { type: 'stopDiploWork'; targetClanId: ClanId; }
export interface CmdProposePact extends CommandBase {       // 提案協定（消耗信用；成功率見 08）
  type: 'proposePact'; targetClanId: ClanId; kind: PactKind;
  termDays: number | null;       // 期限（日）；alliance/ceasefire 必填、marriage/vassal 為 null
}
export interface CmdBreakPact extends CommandBase {         // 毀約（信用/感情懲罰見 08）
  type: 'breakPact'; targetClanId: ClanId; kind: PactKind;
}
export interface CmdDonateCourt extends CommandBase {       // 朝廷獻金
  type: 'donateCourt'; gold: number;                        // >0（貫）→ courtFavor（08）
}
...
| `facility.completed` | `castleId, slotIndex: number, facilityTypeId` | 施設完工 | 05 |
| `policy.enacted` / `policy.revoked` | `clanId, policyId` | 政策生效/廢止 | 05 |
| `conscript.completed` | `castleId, soldiers: number` | 徵兵入營 | 05 |
| `transport.arrived` | `fromCastleId, toCastleId, soldiers: number, food: number` | 輸送抵達 | 05 |
...
| 衍生值 | 公式 / 來源 | 計算時機與快取 |
|---|---|---|
| `castleMaxSoldiers(c)` | `floor( Σ_{d ∈ c.districtIds, d.ownerClanId=c.ownerClanId} d.population × BAL.garrisonPopRatio )`，`BAL.garrisonPopRatio` 建議 0.08（郡被制壓走會即時降低上限） | selector；tick 內 memo |
...
| `fiefCapOf(rank)` | `BAL.fiefMaxByRank`：`{ kumigashira:1, 'ashigaru-taisho':2, 'samurai-taisho':3, busho:4, karo:6, shukuro:8 }` | 常數查表 |
...
| INV-12 | `army.path` 相鄰節點間必有 RoadEdge 相連；`path[pathCursor] = posNodeId`；`0 ≤ pathCursor < path.length`；`0 ≤ edgeProgress < 1`。 |
| INV-13 | `army.status='engaged'` ⇔ `battleId ≠ null` 且該 Battle 的參戰列表含此部隊；`status='sieging'` ⇔ `siegeId ≠ null` 且該 Siege 含此部隊；Battle/Siege 引用的部隊必存在且狀態相符。 |
| INV-14 | `DiplomacyRow.key = pairKey(a,b)` 且 `a < b`（字典序）、`a ≠ b`；`missions` 中同一 `(from,to)` 至多一件、兩端 alive、`officerId` 屬 from 方 serving。 |
| INV-15 | `status='pending'` 的 Proposal：`officerId` 為該勢力 serving 武將、`expiresDay > time.day`、`command.clanId = proposal.clanId`。 |
| INV-16 | 數值範圍：`loyalty/morale/publicOrder/trust*/sentiment*/courtFavor ∈ [0,100]`；`ldr/val/int/pol ∈ [1,120]`；`prestige ∈ [0,2000]`；`commerce ≤ commerceCap`、`kokudaka ≤ kokudakaCap`、`population ≤ populationCap`、`durability ≤ maxDurability`；`gold/food/soldiers/merit ≥ 0` 且為整數。 |
...
| INV-21 | `castle.facilities.length` = 6（main）/ 3（branch）；`buildRemainingDays > 0` ⇒ `typeId ≠ null`。 |
...
```
nextId(state, kind):            // kind ∈ {army,battle,siege,corps,proposal,report}
...
stateHash(state) = fnv1a32(canonicalStringify(state))      // 32-bit FNV-1a，實作於 17
```
...
defaultRow(k): trust 雙向 0、sentiment 雙向 50、pacts []
...
| `BAL.garrisonPopRatio` | 0.08 | 城最大兵力 = 所轄郡人口和 × 此值 |
...
| `BAL.fiefMaxByRank` | 1/2/3/4/6/8 | 各身分知行郡數上限（§5.1 表） |
| `BAL.maxActivePolicies` | 4 | 同時生效政策數上限 |
| `BAL.proposalExpireDays` | 60 | 具申逾期日數 |
| `BAL.reportMaxKept` | 600 | 報告保留上限 |
| `BAL.kassenFieldW` / `BAL.kassenFieldH` | 24 / 16 | 合戰戰場格數 |
| `BAL.kassenMaxRounds` | 60 | 合戰強制結束回合 |
