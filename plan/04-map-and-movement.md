...
- **2026-07-22（[M6-V9] 地圖可讀性——勢力色雙軌與紙墨邊、標籤 pixel-lock/halo/declutter、節點降噪，交叉引用）**：
  (1) 勢力色拆雙軌——旗幟軌 `clanColorHsl`（節點/軍旗/HUD，不變）與染紙軌 `clanDyeHsl`
  （s25、L 奇偶 67/53，僅餵 territory 底色），落實 art-bible「染紙而非油漆」；領地邊界由
  `borderDarken` 乘法加深改為朝 `borderInk`(0x2e281d) 以 `borderInkMix`(0.55) 混合的紙墨邊
  （角落 once-guard 防近黑）。(2) 標籤全面改 per-label container 反向縮放（字級恆 CSS px、
  僅 camera.scale 變時更新、不動 rebuildCounts）＋ink900 字/washi100 halo（`labelStyle.ts`
  LABEL_STYLE）＋64×64 CSS px declutter（idle/scale 變/LOD 變觸發，優先序主城＞國名＞支城＞
  郡＞道路），對齊 art-bible §10。(3) 城耐久環/郡節點降噪（r18/13、寬 2、accentMoss、底環
  ink700；郡 r6/α0.6）。單一真相＝`docs/design/m6-v9-hud-readability.md`；三段 visual
  baseline 隨之重生（獨立 commit）。
...
- **2026-07-17（[M6-V8] 軍隊棋子，交叉引用）**：軍隊 `relation` 於 UI 邊界推導、`heading` 由
  renderer 內插、far 可讀性以 container transform 反向放大——皆不進 core selector；
  詳細裁決見 plan/07 §8 [M6-V8]。
