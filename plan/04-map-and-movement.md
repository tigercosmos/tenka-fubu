...
- **糧盡潰散**（糧盡起算由本 movement 系統執行；潰走行為以 07 §3.4 為單一擁有者，E-62）：`food == 0` 起，每日
...
export interface MarchState {
  ...
  /** 當前邊的有效日數快取（進入邊時計算）。 */
  edgeCostDays: number;
  /** 制壓目標（僅 status==='subjugating'）。制壓進度存於 `District.subjugation.progress`
   *  （0..100，依 02／DDR-9 接力制壓，E-13）；本欄僅記目標郡與本隊快取的 daysRequired（§3.8 公式）。 */
  subjugation?: { districtId: string; daysRequired: number };
}
```
...
movementSystem(state):
  ...
  for army in sortById(state.armies):
    if army.march.status in ('engaged','sieging'): continue        // 由 07 控制
...
onArriveNode(army):
  ...
  if 節點為敵郡 → army.status='subjugating'; 設 march.subjugation(districtId＋daysRequired 依 §3.8)，進度累加於 District.subjugation.progress（E-13）
