...
| `plan/07-military.md` | 出陣編成、野戰／合戰解算、威風、攻城、兵站常數（`BAL.armyFoodPerSoldierPerDay`）。本文件觸發野戰後即移交 07。 |
...
- 通行合法性**每日在移動前重新驗證**（外交狀態可能中途改變）：
  ...
  `report.march.blocked` 報告，等待玩家（或 AI）重新下令。
- 海路邊永遠可通行（海上無領有權）。
...
- `Army` 帶有 `march: MarchState`（§4.4）：目前所在節點或所在邊上的進度。
...
- **兵糧**：部隊出陣攜帶兵糧（出陣時自城扣除，參見 07）。每日消耗
  `soldiers × BAL.armyFoodPerSoldierPerDay`（常數定義權在 07，主表 15；建議 0.005 石/人/日）。
- **糧盡潰散**（canonical，本文件定義）：`food == 0` 起，每日
  士氣 −`BAL.starvationMoraleLossPerDay`（建議 8）、兵力流失
  `floor(soldiers × BAL.starvationDesertRatePerDay)`（建議 0.03）。
  當士氣 ≤ `BAL.routMoraleThreshold`（崩潰閾值，主定義於 07/15；建議 20）時部隊**潰走**：
  強制以 `BAL.retreatSpeedFactor`（建議 1.25）倍速沿最短可通行路徑退回最近我方城，
  途中不制壓、不主動遭遇（被敵方逮到仍會被迫野戰，且潰走部隊士氣以 0 計）。
  兵力 < `BAL.armyDisbandSoldiers`（建議 100）時部隊就地解散（武將返回出發城，參見 07）。
  無任何可退回城時（領土全失）立即解散。
...
- 每日 `daysDone += 1`（制壓進度**不吃**行軍速度修正）。`daysDone ≥ daysRequired` 時：
  郡 `ownerClanId` 翻轉為制壓方、郡治安下降 `BAL.subjugateSecurityHit`（建議 15）、
  發出 `report.subjugate.done`，部隊於次日繼續沿路徑前進。
- **制壓中被攻擊**：敵部隊抵達同節點 → 立即中斷制壓、進入野戰（參見 07）。
  - 制壓方勝（敵退走或全滅）：`daysDone` **保留**，次日繼續累計。
  - 制壓方敗退或潰走：`daysDone` 歸零；該郡不翻轉。
...
1. **同節點**：敵對雙方部隊位於同一節點（含一方停駐、制壓中、圍城中）→ 野戰
   （圍城中被襲即「後詰決戰」，解算參見 07）。該節點上**所有**互為敵對的部隊全部捲入同一場野戰。
...
- 合流（合併兩支我方部隊）是明確的 Command，僅限同節點，定義參見 `plan/07-military.md`。
...
實體欄位（`RoadEdge`、`District.isPort`、`Army.march` 等）最終歸屬 `plan/02-data-model.md`。
...
### 4.2 行軍與制壓狀態（掛在 `Army` 上；`Army` 其餘欄位參見 02/07）

```ts
export type ArmyMoveStatus =
  | 'marching'      // 沿路徑行進中
  | 'subjugating'   // 停在敵郡制壓中
  | 'fighting'      // 野戰中（由 07 控制，movement 系統跳過）
  | 'besieging'     // 攻城中（由 07 控制）
  | 'retreating'    // 奉令撤退（走路徑回城，可再下令）
  | 'routed'        // 糧盡/崩潰潰走（不可控，強制回最近我方城）
  | 'holding';      // 原地待命（含通行被阻）

export interface MarchState {
  status: ArmyMoveStatus;
  /** 完整路徑節點序列（含起點與終點）。holding 且無路徑時為長度 1。 */
  path: MapNodeId[];
  /** 目前所在 path 索引：位於 path[nodeIndex]，或行進於 path[nodeIndex]→path[nodeIndex+1] 邊上。 */
  nodeIndex: number;
  /** 當前邊已累積的進度（日）。位於節點上時為 0。 */
  edgeProgressDays: number;
  /** 當前邊的有效日數快取（進入邊時計算）。 */
  edgeCostDays: number;
  /** 制壓進度（僅 status==='subjugating'）。 */
  subjugation?: { districtId: string; daysDone: number; daysRequired: number };
}
```
...
speedFactor(army) = clamp(
  ...
  × (status=='retreating' || status=='routed' ? BAL.retreatSpeedFactor(1.25) : 1.0),
...
movementSystem(state):
  ...
  for army in sortById(state.armies):
    if army.march.status in ('fighting','besieging'): continue        // 由 07 控制
    consumeFood(army)                                                  // §3.7；糧盡處理
    if army.march.status == 'subjugating': subjugationTick(army); continue
    if army.march.status == 'holding': continue
    if not validateNextStep(army): army.status='holding'; emit report.march.blocked; continue
    ...
    while budget > 0 且未到路徑終點:
      ...
      if edgeProgressDays ≥ edgeCostDays: 抵達 path[nodeIndex+1] → onArriveNode(army)
        （若進入 subjugating/besieging/入城解散 → break）
  // 相 2：遭遇（依 (idA,idB) 字典序檢查 §3.9 規則 1~3）
  for 每對敵對部隊 (a, b): if 觸發 → startFieldCombat(a, b, ...)       // 解算移交 07

onArriveNode(army):
  nodeIndex += 1; edgeProgressDays = 0
  owner ← 節點擁有者; stance ← getStance(army.clanId, owner)
  if 侵入中立領 → declareWar(...)                                      // 參見 08
  if 節點為敵郡 → army.status='subjugating'; 初始化 subjugation（daysRequired 依 §3.8）
  else if 節點為敵城（必為終點）→ army.status='besieging'              // 參見 07
  else if 節點為終點:
...
| `BAL.starvationMoraleLossPerDay` | 8 | 點/日；糧盡士氣流失 |
| `BAL.starvationDesertRatePerDay` | 0.03 | 比例/日；糧盡逃兵 |
| `BAL.armyDisbandSoldiers` | 100 | 人；低於此解散 |
| `BAL.armyFoodPerSoldierPerDay` | 0.005（參考） | 石/人/日；**定義權在 07，主表 15** |
| `BAL.routMoraleThreshold` | 20（參考） | 崩潰閾值；**定義權在 07，主表 15** |
...
| `report.march.arrived` | `{army}抵達{place}。` |
| `report.march.blocked` | `{army}行軍受阻，於{place}待命。` |
| `report.march.starving` | `{army}兵糧耗盡，軍心動搖！` |
| `report.march.routed` | `{army}潰散，向{place}敗走！` |
| `report.march.disbanded` | `{army}潰滅，殘兵四散。` |
| `report.subjugate.begin` | `{army}開始制壓{district}。` |
| `report.subjugate.done` | `{clan}制壓了{district}！` |
| `report.subjugate.interrupted` | `{district}的制壓遭{enemy}阻止。` |

（威風、野戰、攻城相關字串定義於 `plan/07-military.md`；通用 UI 字串於 `plan/13-i18n-strings.md`。）
