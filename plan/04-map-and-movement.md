...
制壓快取（02 §4.8 未收錄之 04 專屬擴充欄位；僅 `status==='subjugating'` 時存在，其餘為 `null`；
E-13 沿用既有裁決）：

```ts
/** Army 的制壓快取子欄位（04 §3.8 專用，非 02 §4.8 canonical 欄位）。 */
export interface ArmySubjugation {
  /** 制壓目標郡（＝ posNodeId，另存以便直接讀取）。 */
  districtId: string;
  /** 抵達當下依 §3.8 公式算定之所需日數快取；制壓期間不重算。 */
  daysRequired: number;
}
// Army.subjugation: ArmySubjugation | null
```

制壓「進度」本身**不**存於此——依 02 DDR-9／E-13，累計於 `District.subjugation.progress`
（0..100，見 §3.8）。
...
onArriveNode(army):
  ...
  if 節點為敵郡 → army.status='subjugating'; 設 army.subjugation={districtId, daysRequired}（daysRequired 依 §3.8 公式，抵達時算定；§4.2 `ArmySubjugation`），進度累加於 District.subjugation.progress（E-13）
