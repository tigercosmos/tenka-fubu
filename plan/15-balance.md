...
1. **BAL 常數主表**（§5.1）：涵蓋全部 21 份文件出現過的每一個 `BAL.*` 常數（共 635 個名稱），
   逐項給定定案值、單位、語意、出處、備註。
...
### 5.1 BAL 常數主表（540 項模擬層常數，依擁有文件分子表）
...
#### 編成出陣・野戰・合戰・戰法・威風・攻城・軍團・兵站（07）（87）
...
- [ ] **T1 建立 `src/core/balance.ts`**：依 §4.1 結構，把 §5.1 主表全部 540 項模擬層常數寫入單一 `readonly` `BAL`（`as const`）。
