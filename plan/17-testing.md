...
| # | 案例 | 輸入（基於 mini） | 期望 |
|---|---|---|---|
| E1 | 月初金錢收入 | 推進至下個月 1 日 | alpha 金錢增量 = Σ(4 郡商業 300) × `BAL.goldPerCommerce`(0.5) − 維持費（公式見 05） |
| E2 | 非月初無收入 | 從 4 月 2 日推進至 4 月 3 日 | 金錢不變（無其他支出時） |
| E3 | 秋收 | 推進跨過 9 月 1 日 | `castle.a1.food` 增量 = Σ轄郡石高 40,000 × `BAL.harvestRate`(0.8) = 32,000 石；8 月 30 日與 9 月 2 日皆無此增量 |
| E4 | 駐軍日糧 | 推進 1 日 | 每城 `food` 減量 = ceil(駐軍 × `BAL.garrisonFoodPerSoldierDay`(0.002))；2,000 兵 → −4 石/日 |
| E5 | 出陣部隊日糧較高 | alpha 出陣 1,000 兵攜糧 500 石，推進 1 日 | 部隊攜行糧 −ceil(1,000 × `BAL.armyFoodPerSoldierDay`(0.006)) = −6 石；城不再為該 1,000 兵扣糧 |
| E6 | 兵力維持費 | 推進至月初 | 金錢支出含 Σ全勢力兵力 × `BAL.upkeepGoldPerSoldierMonth`(0.01) |
| E7 | 赤字行為 | override alpha 金錢 = 0，推進至月初 | 金錢 clamp 為 0（不為負）；發出 `GameEvent` 種類 `economy.deficit`；alpha 全武將忠誠 −`BAL.deficitLoyaltyPenalty`(2)、全城士氣 −`BAL.deficitMoralePenalty`(5) |
| E8 | 赤字懲罰每月至多一次 | 承 E7，同月內再推進數日 | 懲罰不重複套用 |
| E9 | 商業 0 邊界 | override 全郡商業 = 0 | 月初商業收入為 0，金錢僅減維持費 |
| E10 | 城糧盡 | override `castle.a1.food` = 0，推進 3 日 | 該城士氣每日 −`BAL.garrisonStarvationMoralePerDay`(3)，下限 0；`food` 不為負 |

#### 3.4.2 development（`src/core/systems/development.spec.ts`）

| # | 案例 | 輸入 | 期望 |
|---|---|---|---|
| D1 | 領主自動開發 | `dist.a1x` 分封給政務 85 武將，推進 30 日 | 開發度增量 = 30 × `BAL.devDailyBase`(0.05) × (1 + 85 × `BAL.stewardPolWeight`(0.01))（公式見 05） |
| D2 | 直轄較慢 | `dist.a1y` 維持直轄，推進 30 日 | 增量 = 30 × `BAL.devDailyBase` × `BAL.directDevFactor`(0.6)；且 D1 增量 > D2 增量 |
| D3 | 開發度上限 | override 開發度 = `BAL.devCap`(100)，推進 30 日 | 開發度不變（不溢出） |
| D4 | 商業上限 | override 商業 = `BAL.commerceCap`(2000)，推進 60 日 | 商業不再成長（00 §6：商業 0..2000） |
| D5 | 開發帶動產值 | 開發度 30 → 40 的郡 | 石高與商業依 05 公式單調不減；斷言前後關係（大於等於）而非絕對值 |
| D6 | 政務差異 | 兩郡領主政務 80 vs 40，各推進 30 日 | 政務 80 郡的開發度增量嚴格較大 |
| D7 | 低治安抑制 | override 治安 = 10（< `BAL.lowSecurityThreshold`(20)），推進 30 日 | 開發增量乘 `BAL.lowSecurityDevFactor`(0.5)；與治安 70 對照組相比約為其半 |
...
| M2 | 途經敵郡需制壓 | alpha 部隊路徑含敵郡 `dist.b2x` | 進入該節點後停留 `BAL.subjugateBaseDays`(5) 日完成制壓（修正見 04），郡歸屬翻轉後才續行 |
| M3 | 糧盡潰走 | 出陣部隊攜糧 override = 0 | 每日兵力 −floor(兵力 × `BAL.starvationAttritionRate`(0.05))、士氣 −`BAL.starvationMoralePerDay`(10)；士氣 ≤ `BAL.routMoraleThreshold`(20) 當日轉為潰走、自動折返出發城 |
...
| M7 | 士氣潰走 | 野戰中一方士氣先降至 ≤ `BAL.routMoraleThreshold` | 該方潰走：脫離戰鬥、承受追擊損失（見 07）、自動返城且途中不可再下令 |
...
| # | 案例 | 輸入 | 期望 |
|---|---|---|---|
| B1 | 發動門檻 | 雙方合計兵力 < `BAL.battleMinSoldiers`(3000) | 不可發動合戰（Command 被驗證器拒絕），僅野戰自動解算 |
| B2 | 勝敗判定 | 合戰子迴圈跑至一方總大將部隊潰走或殲滅 | 該方敗北；`Battle` 結果物件標記勝方、殲滅比、總大將是否被討取 |
| B3 | 威風（小） | 勝方殲滅比 ≥ `BAL.aweSmallKillRatio`(0.3) 且 < 中門檻 | 結果 `awe = 'small'` |
| B4 | 威風（中／大） | 殲滅比 ≥ `BAL.aweMediumKillRatio`(0.45)／≥ `BAL.aweLargeKillRatio`(0.6) 或敵總大將被討取 | `awe = 'medium'`／`'large'`（判定優先序見 07） |
| B5 | 威風擴散效果 | 大威風、戰場鄰近有敵郡與敵城 | 影響範圍內敵郡歸順翻轉、敵城士氣 −`BAL.aweMoraleHit`(15)（範圍與衰減見 07）；範圍外不受影響 |
...
| S2 | 包圍耗糧 | 同上改包圍 | 守城 `food` 每日消耗 ×`BAL.siegeFoodFactor`(2)；耐久每日減量遠小於強攻（可為 0，見 07） |
| S3 | 落城（強攻） | 推進至耐久 ≤ 0 | 城歸屬翻轉；殘兵與武將處置依 07；發 `report.siege.fallen` 報告 |
| S4 | 開城（糧盡） | 包圍下守城 `food` = 0 且士氣 < `BAL.surrenderMoraleThreshold`(30) | 開城：歸屬翻轉、守軍處置較強攻寬（見 07） |
| S5 | 解圍 | beta 援軍擊破圍城部隊 | 圍城狀態解除、守城士氣 +`BAL.siegeReliefMoraleBonus`(20)（上限 100） |
| S6 | 圍城士氣滲透 | 包圍持續 10 日 | 守城士氣每日 −`BAL.siegeMoraleDecayPerDay`(0.5)（威風效果另計） |
...
| # | 案例 | 輸入 | 期望 |
|---|---|---|---|
| DP1 | 外交工作累積信用 | alpha 對 beta 執行外交工作 3 個月（月費 `BAL.diploWorkGoldPerMonth`(20) 貫） | beta 對 alpha 信用 +3 × `BAL.trustPerWorkMonth`(3)；金錢同步扣款；信用上限 100 |
| DP2 | 同盟接受度 | 信用 ≥ `BAL.allianceTrustMin`(60) 且非交戰 → 提案；信用 40 → 提案 | 前者接受、後者拒絕（完整接受度公式見 08，測試以邊界兩側取值） |
| DP3 | 停戰效力 | 交戰中締結停戰 | `BAL.ceasefireDays`(360) 內對該勢力的出陣 Command 被驗證器拒絕；期滿次日恢復合法 |
| DP4 | 破約懲罰 | 同盟存續中對盟友宣戰 | 對象信用歸 0、alpha 威信 −`BAL.betrayalPrestigePenalty`(100)、其他所有勢力對 alpha 信用 −`BAL.betrayalTrustPenaltyOthers`(20) |
| DP5 | 協定到期 | 同盟自然到期（`BAL.allianceDays`(1800)） | 到期日自動解除、雙方收到報告、不觸發破約懲罰 |
| DP6 | 重複提案非法 | 已有婚姻同盟，再送婚姻提案 | Command 驗證器拒絕，狀態不變 |
| DP7 | 從屬與上貢 | 威信差 ≥ `BAL.vassalPrestigeGap`(500) 的小勢力接受從屬 | 每月上貢 = 其月收入 × `BAL.vassalTributeRate`(0.1)（歸主家）；從屬期間主家對其出陣非法 |
...
| # | 案例 | 輸入 | 期望 |
|---|---|---|---|
| O1 | 知行提升忠誠 | 分封 1 郡給忠誠 70 的武將，推進至月初 | 忠誠月結後 ≥ 70 + `BAL.fiefLoyaltyBonus`(10)（其他項不變時）；上限 100 |
| O2 | 低忠誠出奔 | override 忠誠 = 20（< `BAL.defectionLoyaltyThreshold`(30)） | 每月以 `BAL.defectionChancePerMonth`(0.1) 判定（`rng.misc`）；用 `fixedRng` 分別注入 0.05 與 0.95：前者出奔（武將轉浪人、知行回收）、後者留任 |
| O3 | 功績升格 | override 功績 = `BAL.meritRankThresholds[1]`(500) − 1，再獲 1 點 | 身分 足輕組頭 → 足輕大將；發升格報告；閾值陣列建議 [0, 500, 1500, 3500, 7000, 12000] |
| O4 | 身分權限 | 以足輕大將任命城主 | Command 驗證器拒絕（侍大將以上才可任城主，00 §4）；侍大將任命成功 |
| O5 | 壽命死亡 | override 武將年齡 = 80，固定種子推進 | 於可預期月份死亡（判定公式見 06）；同種子重跑死亡月份相同 |
| O6 | 當主死亡繼承 | 當主死亡 | 依 06 繼承順位產生新當主；全家臣忠誠 −`BAL.successionLoyaltyShock`(15)；無繼承人時勢力滅亡處理（見 10） |
...
兩條 golden，皆為「固定劇本＋固定種子＋零玩家指令＋全勢力 AI」的長程模擬：
...
1. `buildFullState({ seed, allAi: true })` 建初始狀態（`allAi` 使玩家勢力也由大名 AI 接管，
   等同委任全權；Command 佇列每 tick 皆為空陣列——AI 產生的指令走 core 內部路徑，見 03/09）。
...
- **豁免檔案**：`TESTCFG.scanExemptFiles = ['plan/17-testing.md', 'tools/simplified-chars.ts']`
  （兩者必然含黑名單字元本身）。豁免以外**無任何白名單檔案**。
- **三層清單**（字元常數定義見 §5.4；完整字集為 canonical，實作時逐字複製）：
  - **L1 無歧義簡體字**（148 字）：出現即錯誤。這些字元在台灣正體中無合法用途
  ...
  - **L3 日文新字體**（17 字）：気、沢、辺、駅、桜、隣、竜、済、継、戦、単、対、図、広、姫、塩、浜。
...
- **記錄**：core 在每次 `applyCommands` 成功套用 Command 時，附掛 `{ day, seq, command }`
  ...
  溢出時整檔標記 `truncated: true` 且不可作為回歸案例）。紀錄器由遊戲迴圈持有（見 03）。
...
export const TESTCFG = {
  ...
  /** 簡體字掃描豁免檔（相對 repo 根目錄） */
  scanExemptFiles: ['plan/17-testing.md', 'tools/simplified-chars.ts'],
} as const;
```
...
export interface CommandLogFile {
  ...
  truncated: boolean;      // 環形紀錄器曾溢出（true 則不可作回歸案例）
...
export interface TestDebugApi {
  ...
  replayCommandLog(log: CommandLogFile): ReplayResult;
}
...
/** L3：日文新字體（17 字）——出現即錯誤。
 *  對應正體（同序）：氣澤邊驛櫻鄰龍濟繼戰單對圖廣姬鹽濱 */
export const SHINJITAI_L3: string = '気沢辺駅桜隣竜済継戦単対図広姫塩浜';
...
export const CONTEXT_L2: ReadonlyArray<{ char: string; allow: RegExp; suggestion: string }> = [
  ...
  { char: '里', allow: /[公海]里|里程|鄉里|里山/u, suggestion: '裡' },
];
```
