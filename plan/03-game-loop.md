...
事件**不持久化**；持久化的是 `ReportEntry`（§3.4.3）。
...
- 發：`event.historicalTriggered`、`event.genericTriggered`、`uprising.broke`。
...
- 發：`diplomacy.envoyArrived`、`diplomacy.pactSigned`、`diplomacy.pactExpired`、`court.rankGranted`、
  `plot.succeeded`、`plot.failed`。
- 詳見：`plan/08-diplomacy.md`。

**Step 5 `development`** — 頻率：每日（小步進）。
- 讀：`state.districts`、`state.officers`（領主能力）、`state.clans`（政策修正）、`rng.dev`。
...
- 發：`economy.income`、`economy.upkeepUnpaid`、`economy.harvested`、`economy.foodShortage`。
...
- 發：`military.armyDeparted`、`military.armyArrived`、`military.encounter`、
  `military.subjugated`、`military.districtLost`（我方視角）。
...
- 發：`combat.battleAvailable`、`combat.fieldResolved`、`siege.started`、`siege.progress`、
  `siege.fallen`、`siege.repelled`、`battle.won`、`battle.lost`、`battle.aweTriggered`（AI 對 AI 自動解算時）。
- 詳見：`plan/07-military.md`。

**Step 9 `officers`** — 頻率：每日呼叫；忠誠變化與壽命死亡判定閘控每月 1 日；元服每年 1 月 1 日。
- 讀：`state.officers`、`state.clans`、`rng.event`（死亡判定）、`rng.misc`（忠誠雜項）。
- 寫：`state.officers`（忠誠、年齡、生死、出奔）、`state.clans`（當主死亡繼承）。
- 發：`officer.loyaltyLow`、`officer.defected`、`officer.died`、`officer.comingOfAge`、`officer.rankPromoted`。
...
- 發：`victory.achieved`、`defeat.playerEliminated`、`clan.eliminated`。
- 詳見：`plan/10-events-and-victory.md`。

**Step 13 `reports`** — 頻率：每日。
- 讀：本 tick 事件陣列、`state.time`、玩家勢力 ID。
- 寫：`state.reports`（追加 `ReportEntry`、修剪保留期）；計算 `TickResult.autoPauseReasons`。
- 發：無（本步是事件的終端消費者）。
- 詳見：§3.4。
...
- 失敗回饋：發出 `command.rejected` 事件（severity `warning`），內含 `reasonKey`（i18n key）
  與插值參數；Step 13 轉為 `ReportEntry`，UI 以 toast 呈現（§6）。
...
| CommandType | 擁有文件 | 主要驗證重點 | 重複提交語意 |
|---|---|---|---|
| `cmd.developDistrict`（郡開發指示） | 05 | 直轄郡、金錢足夠 | 覆蓋（改開發方針） |
| `cmd.assignSteward`（知行任命領主） | 05, 06 | 郡屬我方、武將身分與知行上限 | 覆蓋（換領主） |
| `cmd.revokeSteward`（收回直轄） | 05 | 郡有領主 | 拒絕 `alreadyActive`（已直轄） |
| `cmd.buildFacility`（城下施設） | 05 | slot 空、金錢足夠 | 拒絕 `alreadyActive`（同 slot 建設中） |
| `cmd.conscript`（徵兵） | 05 | 人口、金錢、治安下限 | 疊加（多次徵兵各自結算） |
| `cmd.transport`（輸送兵糧/兵力） | 05 | 來源城庫存、路徑連通 | 疊加（每筆為獨立輸送隊） |
| `cmd.adoptPolicy`（施行政策） | 05 | 威信門檻、金錢、互斥政策 | 拒絕 `alreadyActive` |
| `cmd.repealPolicy`（廢止政策） | 05 | 政策施行中 | 拒絕 `invalidTarget` |
| `cmd.march`（編成出陣） | 07 | 城內兵力、大將指定、目標節點 | 疊加（可多路出兵） |
| `cmd.setArmyPath`（改路徑/目標） | 07 | 部隊屬我方、路徑合法 | 覆蓋 |
| `cmd.recallArmy`（撤收） | 07 | 部隊屬我方 | 拒絕 `alreadyActive`（已在回程） |
| `cmd.initiateBattle`（發動合戰） | 07 | `combat.battleAvailable` 旗標存在、兵力門檻 | 拒絕 `invalidTarget`（機會已逝） |
| `cmd.setSiegeMode`（強攻/包圍切換） | 07 | 我方為圍城方 | 覆蓋 |
| `cmd.formCorps`（設立軍團） | 07 | 軍團長身分家老以上、城群連通 | 拒絕 `alreadyActive` |
| `cmd.dissolveCorps`（解散軍團） | 07 | 軍團存在 | 拒絕 `invalidTarget` |
| `cmd.diplomacyWork`（外交工作） | 08 | 金錢、使者武將空閒 | 拒絕 `alreadyActive`（同對象進行中） |
| `cmd.proposePact`（提案協定） | 08 | 信用門檻、無互斥協定 | 拒絕 `alreadyActive` |
| `cmd.respondPact`（回應來使） | 08 | 存在待回應提案 | 拒絕 `invalidTarget`（已回應） |
| `cmd.courtAction`（朝廷/幕府工作） | 08 | 金錢、官位前置 | 拒絕 `alreadyActive` |
| `cmd.plot`（調略：引拔/流言/內應） | 08 | 執行武將知略門檻、金錢 | 疊加（不同目標）；同目標拒絕 |
| `cmd.recruitOfficer`（登用） | 06 | 目標為浪人或俘虜、金錢 | 拒絕 `alreadyActive` |
| `cmd.rewardOfficer`（褒賞） | 06 | 金錢/家寶庫存 | 疊加 |
| `cmd.appointCastleLord`（任命城主） | 06 | 身分侍大將以上 | 覆蓋 |
| `cmd.releaseOrExecute`（俘虜處置） | 06 | 俘虜存在 | 拒絕 `invalidTarget` |
| `cmd.respondProposal`（裁可/駁回具申） | 06 | 具申未逾期 | 拒絕 `invalidTarget`（已結案） |
| `cmd.issueTaimei`（發動大命） | 10 | 大命點數/條件 | 拒絕 `alreadyActive` |
| `cmd.respondEvent`（歷史事件選擇） | 10 | `pendingChoice` 存在且選項合法 | 拒絕 `invalidTarget` |
| `cmd.debugSkipDays`（時間跳轉） | 03（本文件 §3.9） | debug 模式 | 疊加 |
| `cmd.debugGrant`（資源作弊） | 03（本文件 §3.9） | debug 模式 | 疊加 |
...
| `combat.battleAvailable`（玩家可發動） | critical | `battleAvailable` | 00 §5.2 列名 |
| `proposal.submitted` | info | `proposalArrived` | 具申送達（00 §5.2 列名） |
| `diplomacy.envoyArrived` | warning | `envoyArrived` | 外交來使（00 §5.2 列名） |
| `event.historicalTriggered` | critical | `historicalEvent` | 開 modal（00 §5.2 列名） |
| `battle.won` / `battle.lost` | critical | —（modal 已凍結時間） | 我方參戰時 |
| `siege.fallen` | 我方城陷落 critical；他勢力 info | — | |
...
| `uprising.broke` | 我方領內 warning；他勢力 info | — | |
| `diplomacy.pactExpired` / `diplomacy.pactBroken` | 涉我方 warning；否則 info | — | |
| `clan.eliminated` | info（涉我方另走 `defeat.*`） | — | |
| `victory.achieved` / `defeat.playerEliminated` | critical | —（進結局畫面，見 10/11） | |
...
reportsSystem(state, events) -> AutoPauseReason[]:
  for e of events:                          // 依發出順序
    sev  = severityOf(e, state.meta.playerClanId)   // §3.4.2 表
    if isPlayerRelevant(e) or sev !== 'info':
      state.reports.push(makeReportEntry(e, sev))   // i18n key 映射見 13
  // 修剪：保留最近 BAL.reportRetentionDays = 360 日內，且總數 ≤ BAL.reportMaxEntries = 500
  //（超量時先丟最舊的 info 級，再丟最舊的 warning 級；critical 不受總數修剪、僅受日數修剪）
  return dedupedAutoPauseReasons(events, state.settings)
```

- 純他勢力之間的 info 事件（如 AI 互相開發）**不產生** ReportEntry，只在事件陣列供
  UI 當日 ticker 顯示（若 UI 有訂閱），避免報告匣被灌爆。
- `ReportEntry.linkTarget` 讓 UI 點擊跳轉至相關城／武將／部隊（11 定義跳轉行為）。
...
| 流名 | 用途（唯一允許的消費者） |
|---|---|
| `battle` | Step 8 野戰/攻城解算、合戰子迴圈（§3.7） |
| `dev` | Step 5 郡開發成長判定 |
| `ai` | Step 10 具申生成、Step 11 AI 決策 |
| `event` | Step 3 事件判定、Step 4 外交工作成敗、Step 9 壽命死亡 |
| `misc` | 其餘雜項（遭遇細節、忠誠擾動、浪人生成……） |
...
Step 4  diplomacy : (8) 協定到期 — 各協定 remainingMonths −= 1；歸零者失效，
                                   發 diplomacy.pactExpired（延長規則見 08）
...
- 玩家於 `combat.battleAvailable` 後下達 `cmd.initiateBattle`；Step 1 套用時建立
  `Battle`（status `'deploying'`）並凍結時間。**同一 tick 的其餘步驟照常跑完**
  （合戰發生在「該日之內」，該日其他系統照常運作）。
...
```
  ...
  （設 `territoryChangedToday`）、發 `battle.won`/`battle.lost`/`battle.aweTriggered`/
...
- Step 3 觸發需要選擇的事件時，寫入 `state.events.pendingChoice` 並發
  `event.historicalTriggered`（critical、自動暫停、開 modal）。
- 玩家選擇 → `cmd.respondEvent` 入佇列 → 玩家按播放 → 下一 tick Step 1 套用選項效果
...
| `'preBattle'` | `cmd.initiateBattle` 套用成功、modal 開啟前 | 每次玩家合戰 |
...
- 跳轉結束發 `command.rejected` 以外的專屬報告？否——跳轉本身不產生報告；
  期間各系統照常產生的 ReportEntry 全數保留，玩家可事後翻閱。
- 決定論：跳轉走與正常播放完全相同的 `advanceDay`，golden test 可用它快速推進。

`cmd.debugGrant { gold?, food?, castleId? }`：debug 模式下直接加資源，用於測試；
...
/** 聯集（各成員 payload 見擁有文件；此處列舉判別值） */
type CommandType =
  | 'cmd.developDistrict' | 'cmd.assignSteward' | 'cmd.revokeSteward'
  | 'cmd.buildFacility' | 'cmd.conscript' | 'cmd.transport'
  | 'cmd.adoptPolicy' | 'cmd.repealPolicy'
  | 'cmd.march' | 'cmd.setArmyPath' | 'cmd.recallArmy'
  | 'cmd.initiateBattle' | 'cmd.setSiegeMode'
  | 'cmd.formCorps' | 'cmd.dissolveCorps'
  | 'cmd.diplomacyWork' | 'cmd.proposePact' | 'cmd.respondPact'
  | 'cmd.courtAction' | 'cmd.plot'
  | 'cmd.recruitOfficer' | 'cmd.rewardOfficer' | 'cmd.appointCastleLord'
  | 'cmd.releaseOrExecute' | 'cmd.respondProposal'
  | 'cmd.issueTaimei' | 'cmd.respondEvent'
  | 'cmd.debugSkipDays' | 'cmd.debugGrant';
...
type GameEventType =
  ...
  | 'command.rejected'
  // 經濟
  | 'economy.income' | 'economy.upkeepUnpaid' | 'economy.foodShortage' | 'economy.harvested'
  // 內政
  | 'development.completed' | 'development.districtGrown'
  // 軍事
  | 'military.armyDeparted' | 'military.armyArrived' | 'military.encounter'
  | 'military.subjugated' | 'military.districtLost'
  | 'combat.battleAvailable' | 'combat.fieldResolved'
  | 'battle.started' | 'battle.won' | 'battle.lost' | 'battle.aweTriggered'
  | 'siege.started' | 'siege.progress' | 'siege.fallen' | 'siege.repelled'
  // 武將
  | 'officer.loyaltyLow' | 'officer.defected' | 'officer.died' | 'officer.comingOfAge'
  | 'officer.rankPromoted' | 'officer.captured'
  // 具申
  | 'proposal.submitted' | 'proposal.expired'
  // 外交/朝廷/調略
  | 'diplomacy.envoyArrived' | 'diplomacy.pactSigned' | 'diplomacy.pactExpired'
  | 'diplomacy.pactBroken' | 'court.rankGranted' | 'plot.succeeded' | 'plot.failed'
  // 事件/勝敗
  | 'event.historicalTriggered' | 'event.genericTriggered' | 'uprising.broke'
  | 'victory.achieved' | 'defeat.playerEliminated' | 'clan.eliminated';
// 擴充規則：新增事件必須同步登錄 §3.4.2 分級表與 13 的 i18n 對照。

type GameEvent = GameEventBase & { payload: Record<string, string | number> };
// payload 內容由發出系統的文件定義（例：battle.won 的 payload 見 07）。
// key 一律對應 i18n 插值參數名（§6 / 13）。

type ReportSeverity = 'info' | 'warning' | 'critical'; // 情報/警告/重大

/** 持久化的通知（存於 GameState.reports，陣列依 day 升冪） */
interface ReportEntry {
  id: string;                  // `rep.{absoluteDay}.{當日流水號}`
  day: number;                 // 發生日 absoluteDay
  severity: ReportSeverity;
  messageKey: string;          // i18n key（事件→key 映射表見 13）
  params: Record<string, string | number>; // 插值參數（顯示名等）
  linkTarget?: { kind: 'castle' | 'district' | 'officer' | 'clan' | 'army' | 'battle'; id: string };
  read: boolean;               // 玩家已讀（UI 置換；cmd 不需要，由 UI 直寫允許——唯一例外欄位）
}
```
...
trimReports(reports):
  ...
  移除 day < cutoff 的全部項目
  while reports.length > BAL.reportMaxEntries:                // = 500
...
| `BAL.reportMaxEntries` | 500 | 筆 | 報告總量上限（§3.4.3） |
...
（各事件 → 報告文字的完整映射，如 `report.battle.won`、`report.siege.fallen`，由 13 主表定義。）
...
- [ ] **T5　GameEvent 匯流排與 reports**：severity 表、`ReportEntry` 生成、修剪演算法、
  `autoPauseReasons` 去重。
...
- [ ] **T11　debug 跳轉與自動存檔 hook**：`cmd.debugSkipDays`、`cmd.debugGrant`、
...
- **D6　事件不持久化、Report 才持久化**：GameEvent 是 tick 內的通訊媒介，量大且多數
  與玩家無關；持久化層只保留玩家可讀的 `ReportEntry` 並設保留期與總量上限，
  控制存檔體積。
...
- **D11　`ReportEntry.read` 允許 UI 直寫**：已讀狀態純屬介面便利，不影響任何模擬結果，
