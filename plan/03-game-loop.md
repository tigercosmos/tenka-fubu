...
- 發：`time.monthStart`（1 日）、`time.seasonStart`（季初）。
  月末（晦日）、年初（1/1）、秋收（9/1）不發 GameEvent——對應結算改由各步以 `state.time`
  日期直接閘控（協定到期 Step 4、年結 Step 9、秋收 Step 6；二輪裁決 C）。
...
- 發：`development.completed`（單項開發滿級）。（不發 `development.districtGrown`——13 null-report 無消費者，二輪裁決 C。）
...
- 發：`army.departed`、`army.arrived`、`district.subjugated`。
...
Step 2  time      : 進入晦日（day===30）；月末不發 GameEvent（協定到期由 Step 4 以日期閘控）
...
type GameEventType =
  ...
  | 'command.rejected'
  // 經濟（economy.income／economy.harvest 依 02 §4.19；upkeepUnpaid／foodShortage 為 05 事件、13 §6.11 報告消費，02 §4.19 未列）
  | 'economy.income' | 'economy.upkeepUnpaid' | 'economy.foodShortage' | 'economy.harvest'
  // 內政（development.completed 為 05 開發事件、13 §6.11 報告消費，02 §4.19 未列）
  | 'development.completed'
  // 軍事（依 02 §4.19：army.departed／army.arrived／district.subjugated／battle.*／siege.*／awe.triggered）
  | 'army.departed' | 'army.arrived' | 'district.subjugated'
...
timeSystem(state, emit):
  ...
  // 年初（1/1）、秋收（9/1）、月末（30 日）不發 GameEvent；對應結算由各步以
  // t.day／t.month 直接閘控（年結 Step 9、秋收 Step 6、協定到期 Step 4；二輪裁決 C）
```
