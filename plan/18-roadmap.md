...
- 風險清單與緩解排程。
- M6 期間優先執行的 `M6-V` 視覺基礎工作串流：原創美術規範、地圖圖層、圖示、素材管線與視覺品質門檻。
...
| `plan/04-map-and-movement.md` | §7 任務拆為 M2（渲染／尋路）、M4（行軍／制壓／遭遇）與 M6-V（地形、道路、城池、軍隊、圖層與 dirty update），見 §8-D2／D19 |
...
| `plan/11-ui-screens.md` | §7 任務依所屬系統分散於 M1–M8；M6-V 先完成主畫面資訊層級、HUD safe area 與原創美術規範 |
| `plan/12-ui-components.md` | §7 任務依消費時點分散於 M1–M8；M6-V 先完成圖示語彙、素材 manifest／atlas 與既有 HUD 元件接線 |
...
| `plan/17-testing.md` | §3.11.2 品質門檻表為每個里程碑 DoD 的必要條件；M6-V 增加固定展示戰局、三段縮放截圖與渲染效能／可讀性門檻 |
...
M0 鷹架 ──► M1 core骨架＋最小HUD ──► M2 資料子集＋地圖 ──► M3 內政 ──► M4 軍事(自動解算)
                                                                        │
        M9 平衡/效能/發版 ◄── M8 事件/存檔/全國資料 ◄── M7 AI ◄── M6 外交/家臣＋M6-V 視覺基礎 ◄── M5 合戰
```
...
- `milestone.json.current === "M6"` 期間，`M6-V` 是最高優先工作串流；除已在進行中的安全收尾外，
  其餘 M6 任務暫停開工，直到 `M6-V11` 與 `M6-V` DoD 通過。M6 checkpoint 與 M7 開工亦受此 gate 阻擋。

### 3.2 任務規格格式

- **任務 ID**：一般任務為 `M{里程碑}-{序號}`（如 `M3-8`）；里程碑內的阻斷工作串流可用
  `M{里程碑}-{代號}{序號}`（如 `M6-V3`）。commit 與 PR 引用此 ID（§3.13）。
...
1. 手動流程：織田對齋藤出陣→制壓一郡→野戰勝→圍稻葉山→落城→城與轄郡翻轉、報告正確。
   因 M7 AI 尚未落地且 s1560 初始無在外部隊，本流程使用 `?debug=1` 的決定論 setup：以真實 Command
   建立一支受控齋藤部隊，並提供一個織田可先行制壓的齋藤郡路徑；setup 後不得直接改寫 GameState，
   所有推進均走正常 Command 與 daily tick。
...
元服/具申結算）落地；外交畫面、具申收件匣、家臣團 UI 完成。M6 開始時先以 `M6-V` 清償已完成
M2–M5 的視覺整合債，讓地形、道路、城池、軍隊與核心 HUD 達到可讀、可操作、可持續擴充的基線。

**開工前必讀**：`08`（全文）、`06` §3.5–§3.8、`07` §3.11（內應接點）、`11` §3.3／§3.8–§3.10／§3.15、
`04` §3.10–§3.13、`12` §3.1–§3.3、`17` §3.8–§3.9。

#### 3.9.1 M6-V — 視覺基礎與地圖圖層整合（M6 checkpoint 阻斷）

**目的**：把目前「程式化占位圖形＋未接線元件」提升為第一個可當正式遊戲畫面的 visual slice。
這不是回滾 M2–M5；既有任務仍視為技術交付完成，M6-V 專門清償接線、素材、美術一致性與視覺驗收債。

**現況基線（2026-07-14）**：

- `MapRenderer` 已有 8 個容器，但 `territory` 尚未掛上 `TerritoryGrid` 紋理；背景仍為單色海面與陸地 polygon。
- `CastleNode`／`DistrictNode` 已完成，production 仍使用 `drawNodeMarker` 占位；道路只有端點直線，far LOD 會整層隱藏。
- `ArmyChip` 已接線，但 s1560 新局 `armies` 為空；視覺驗收必須另有固定出陣中的展示戰局。
- `ResourceBar`、`SpeedControl`、`MiniMap`、`IconButton` 已存在，`MainScreen` 尚未組裝，仍以 inline style 與文字按鈕為主。
- 尚無圖像素材 manifest／授權資料／atlas build／Pixi texture 生命週期，也沒有固定截圖基準。

**原創美術方向**：採「低飽和戰國沙盤 × 現代戰略資訊層」。地形是第一閱讀層，道路與城池是第二層，
軍隊與命令是第三層，HUD 只承載時間、資源、警報與情境操作。和紙、墨、木、漆與金箔只作材質語彙，
不可犧牲字級、對比與點擊範圍。所有勢力與狀態除色彩外，還須有旗型、線型、輪廓或文字的第二辨識通道。

**參考圖使用規則**：可研究《信長之野望・新生》公開畫面中「地形高低差、道路網路、城池資訊錨點、
部隊行軍線、補給／到達時間與情境面板」的資訊層級；不得複製其版面比例、面板位置、icon 輪廓、
武將肖像、家紋、字型、城模、貼圖、配色數值或動效。每張 mood board 圖必須記錄來源 URL、存取日期、
抽象觀察與「不可直接沿用」欄。主要研究入口：

- KOEI TECMO 官方 [System](https://www.koeitecmoamerica.com/nobunaga/awakening/system/)、
  [Main Screen](https://www.koeitecmoamerica.com/manual/nobunaga/awakening/en/4100.html)、
  [Map Info](https://www.koeitecmoamerica.com/manual/nobunaga/awakening/en/4200.html)。
- 官方 [Marching](https://www.koeitecmoamerica.com/manual/nobunaga/awakening/en/7100.html)、
  [During Battles](https://www.koeitecmoamerica.com/manual/nobunaga/awakening/en/7400.html) 與
  [繁中會戰介紹](https://www.gamecity.com.tw/shinsei/battle.html)。

**目標圖層堆疊（由下而上；canonical 細節見 04 §3.10.1）**：

| # | layer key | 玩家讀到的內容 | 更新策略 |
|---|---|---|---|
| 0 | `seaBackground` | 海色、海岸外背景 | init 一次 |
| 1 | `terrainBase` | 平原、山地、森林、海岸陰影與近景紋理 | init／terrain pack 變更 |
| 2 | `waterFeatures` | 河川、湖泊、海峽與橋樑下方水體 | init 一次 |
| 3 | `territory` | 勢力領域、郡界與模式化色彩覆蓋 | owner dirty |
| 4 | `analysisOverlay` | 補給、通行、地形優勢、控制範圍等可切換分析層 | 模式／選取 dirty |
| 5 | `roads` | 大道、小路、海路、橋樑、阻斷與路徑高亮 | road data／狀態 dirty |
| 6 | `settlements` | 城下町、農田、港口等低對比地景 | init／城等級 dirty |
| 7 | `nodeMarkers` | 城池、關隘、郡節點與耐久／警戒 | castle/district dirty |
| 8 | `armies` | 軍旗、兵力、士氣、補給與行軍方向 | 每幀只更新移動部隊 |
| 9 | `selectionAndPath` | 選取、命令、路徑、日刻度與目的地 | interaction dirty |
| 10 | `effects` | 戰鬥、攻城、威風與低成本環境回饋 | event driven |
| 11 | `labels` | 城名、道路名、勢力名、ETA 與警告 | camera／locale dirty |
| 12 | `debug` | 圖層邊界、命中區、視錐與效能資訊 | DEV only |

**LOD 資訊層級**：overview（約 25%）保留地形輪廓、主要水系、主幹道、本城、軍隊、勢力名；
operational（約 50%）加入次要道路、支城、郡界、城名與行軍狀態；close（約 100% 以上）才加入聚落、
森林細節、道路名、耐久、士氣與補給細節。切換須有 10% hysteresis 或等效節流，避免縮放臨界值閃爍。

| ID | 任務 | 規格依據 | 主要產出 | 依賴 | 規模 | 驗收標準 |
|---|---|---|---|---|---|---|
| M6-V1 | 原創 Art Bible＋reference board：色盤、材質、光照、三段 LOD、城池／軍隊視角、HUD safe area、icon grid、禁抄清單 | 11 §3.15、12 §3.1 | `docs/visual/art-bible.md`、`docs/visual/references.md` | — | M | 五類地圖物件與六類 UI icon 各有原創規格；每張參考圖具來源／抽象觀察／禁用欄 |
| M6-V2 | 固定 visual fixture＋截圖 harness：有地形、道路、兩類城、至少 8 支敵我軍、圍城、補給警告與三段縮放 | 17 §3.9.3 | `src/core/debugVisual.ts`、`e2e/visual.spec.ts`、`tests/visual/*` | M6-V1 | L | 1280×720、DPR 1、固定字型／種子下 overview／operational／close 基準可重現 |
| M6-V3 | 素材管線：manifest、來源／授權 metadata、命名、atlas build、壓縮、尺寸預算、Pixi preload/cache/dispose | 12 §3.7 | `src/ui/assets/*`、`tools/build-atlas.ts`、`public/assets/*` | M6-V1 | L | 未登錄或缺授權素材使驗證失敗；重掛 renderer 無 texture/cache 洩漏；production 無 raw source 圖 |
| M6-V4 | view contract 與 dirty update：保留道路 `name/waypoints`、城耐久、軍隊士氣／補給／方向、camera command；停止每 tick 全量重畫靜態層 | 04 §4.6 | `selectors.ts`、`mapViewTypes.ts`、`MapRenderer.ts` | M6-V2 | L | 無變更 tick 不重建 terrain/roads/nodes/labels；owner 變更只更新 territory；移動只更新相關 ArmyChip |
| M6-V5 | 地形／水系／領地：山地、森林、平原、河流、湖泊、海岸陰影與三級 LOD；把既有 `TerritoryGrid` 接成 Sprite | 04 §3.10.1–§3.10.3 | `src/ui/map/terrain/*`、`MapRenderer.ts` | M6-V3, M6-V4 | L | 無面板時可辨主要山脈／水系／陸地；領地翻轉下一幀更新；far view 不退回單色 polygon |
| M6-V6 | 道路／橋樑：waypoints 多段線、casing、道級線寬／線型、主幹道 far LOD、道路名、選取高亮與擴大命中容錯；補 visual fixture 與東海／近畿主幹道 waypoints／橋樑標記 | 04 §3.4／§3.10 | `road.ts`、`gameState.ts`、`roads.json`、`mapDraw.ts` | M6-V4, M6-V5 | L | overview 可見主幹道；三道級與海路不只靠顏色辨識；fixture 無端點直線穿山／穿海；小路／節點可用至少 32 CSS px 命中區操作 |
| M6-V7 | 城池／郡／聚落：production 接上既有 `CastleNode`／`DistrictNode`，加入平城／山城輪廓、耐久／受攻／警戒與城下低對比地景 | 04 §3.10、12 §3.3 | `sceneParts/castleNode.ts`、`MapRenderer.ts` | M6-V3, M6-V4, M6-V5 | L | overview 3 秒內辨識本城與歸屬；operational 可分平城／山城與狀態；不再呼叫占位 `drawNodeMarker` |
| M6-V8 | 軍隊棋子：沿用 `ArmyChip` 契約換原創 sprite／旗型，加入方向、士氣、補給與狀態 badge；處理疊放與標籤避讓 | 07、12 §3.3.1 | `sceneParts/armyChip.ts`、atlas | M6-V3, M6-V4 | L | 8 支以上交錯時仍能辨識敵我、方向與選取對象；色弱模擬下不只靠紅綠；visual fixture 開場即可見軍隊 |
| M6-V9 | 核心 HUD 與圖示接線：組裝既有 `ResourceBar`、`SpeedControl`、`MiniMap`、`IconButton`；建立一致 icon family、情境面板與可收合通知 | 11 §3.3／§3.15、12 §3.2 | `MainScreen.tsx`、`components/*`、`icons.tsx` | M6-V1–M6-V4 | L | `MainScreen` 不再以文字按鈕／inline style 為主要 UI；1280×720 地圖仍為視覺主體；MiniMap 可驅動 camera |
| M6-V10 | 戰略分析圖層與圖例：勢力／郡界、通行能力、補給、地形優勢、城池防禦／角色；支援開關、透明度與快捷鍵 | 04 §3.10／§3.13、11 §3.16 | `analysisOverlay/*`、`LayerControl.tsx` | M6-V4–M6-V9 | L | 任一時刻最多一個分析主題高亮；圖例與地圖同步；切換不改 GameState 且 100ms 內有回饋 |
| M6-V11 | 視覺、效能與無障礙 gate：三縮放 screenshot、layer-presence smoke、字級／對比／色弱、點擊容錯、texture memory 與 55fps | 17 §3.9.2–§3.9.3 | CI／測試／檢查表 | M6-V5–M6-V10 | M | 固定基準全綠；20 支移動軍隊 1080p ≥55fps；重掛／縮放無 WebGL context lost；無 critical a11y violation |

**M6-V Definition of Done**：

1. 不開面板即可看出主要山脈、水系、主幹道、城池與軍隊；overview／operational／close 各有清楚資訊層級。
2. 受測者或 reviewer 在 3 秒內能指出敵我軍、城池歸屬、主要行軍方向與目前選取物；不得依賴顏色作唯一線索。
3. 固定 visual fixture 的三張 baseline 與 layer-presence smoke 全綠；1280×720 無標籤全面重疊、HUD 不遮主要戰場。
4. 道路節點與小 icon 使用擴大隱形 hit area；鍵盤焦點、tooltip、最低字級與對比符合 12／17 的無障礙基線。
5. 1920×1080、20 支移動部隊、連續縮放／平移 30 秒維持 ≥55fps；texture cache 與 heap 在 renderer 重掛後回到穩態。
6. 素材 manifest 內每筆皆有作者／來源／授權／衍生狀態；不存在 KOEI TECMO 或其他遊戲的直接擷取資產。

| ID | 任務 | 規格依據 | 產出檔案 | 依賴 | 規模 | 驗收標準 |
|---|---|---|---|---|---|---|
| M6-1 | 外交資料結構與初始化（DiplomacyRow/CourtState） | 08-T1 | `src/core/systems/diplomacy.ts` | M6-V11 | M | 鏡像不變量成立 |
...
| M6-14 | 家臣 UI（家臣團/詳情/褒賞/捕虜/浪人登用） | 06-T11 | `src/ui/screens/*` | M6-10, M3-21, M6-V9 | L | smoke：完成一次小額賞賜 |
| M6-15 | 外交畫面＋朝廷幕府頁＋調略精靈＋來使 modal | 08-T9、11-T8 外交/政策部分 | `src/ui/screens/DiplomacyScreen.tsx` 等 | M6-4, M6-7, M6-V9 | L | disabled 有原因 tooltip；來使自動暫停 |
| M6-16 | 具申收件匣 | 11-T9 | `src/ui/screens/ProposalInbox.tsx` | M6-13, M6-V9 | M | 卡片流；處理完接續月初佇列 |
...
**M6 DoD**：
1. `M6-V` 六條 DoD 全數通過；三段 visual baseline、素材驗證、55fps 與無障礙 gate 為阻斷項。
2. `npm test` → diplomacy/officers 案例（17 §3.4.6/§3.4.7）與兩條 golden 全綠。
3. 手動：與武田締結同盟→請求援軍；對今川武將引拔成功一次；一件具申完整走完採納流程。
4. 17 §3.11.2 M6 欄全 ✓。執行 §3.14 checkpoint。
...
| R9 | **視覺整合債**：地形／領地未接線、正式 scenePart 與 HUD 元件成為 dead code，功能持續堆在占位畫面上 | M8 才補畫面時會同時遇到素材、契約、效能與版面重構 | M6 checkpoint 前插入阻斷 `M6-V`；先做 fixture／素材管線／dirty update，再依地形→道路→城池→軍隊→HUD 拓撲交付，每步皆有截圖與 layer-presence gate |
| R10 | **參考圖與素材權利**：把「致敬」誤作描摹、或未記錄 AI／公有領域／相容授權素材來源 | 法務風險、資產需整批撤換 | Art Bible 明訂只萃取資訊層級；manifest 強制作者／來源／授權／衍生狀態；缺欄即 CI 失敗；禁止直接擷取 KOEI TECMO 與其他遊戲資產 |
| R11 | **紋理與圖層效能**：在現有每 tick 全量重畫上直接疊大型 texture／label | FPS 與記憶體惡化、行動裝置崩潰 | M6-V4 先拆 dirty domain 與 texture 生命週期；atlas／bundle 設尺寸預算；M6-V11 以 20 支移動軍隊、重掛與連續縮放作阻斷 gate |
...
本文件不重複定義遊戲內 UI 細節；`M6-V` 的畫面、圖層、元件與測試內容分別以 04、11、12、17 為
單一真相來源，本文件只擁有施工順序、依賴與整合驗收。`M6-V10` 新增的圖層控制字串須依 13 的 key
規範落入 `src/i18n/zh-TW.ts`，不得在 roadmap 另立字串主表。
...
- **D15｜M6-7 驗收標準回歸修正（2026-07-11，回歸修復）**：內應發動效果之單一真相已改採
  ...
  除 M6-7 本項外無需其他修正。
- **D16｜M0 開工適配：repo 已存在，跳過 §3.3「第一步具體指令」的子目錄新建與 `git init`
  （2026-07-11，實作適配）**：§3.3「第一步具體指令」的指令塊假設「全新倉」情境
  （`npm create vite@latest tenka-fubu` 建子目錄、`git init`）。實際開工時 repo
  （`/Users/tigercosmos/nobunaga_ambition`）已存在且已有 `plan/`、`README.md`、`.git`
  （即本計畫文件本身所在倉）。故 M0-2 改為直接在 repo 根目錄鋪設 `package.json`／
  `vite.config.ts`／`tsconfig*.json`／`src/**` 等檔案，**不**執行子目錄建立與 `git init`，
  其餘任務內容（目錄骨架、工具鏈設定、npm scripts 全表）與驗收標準不變。
  另：`validate:data` 的 scan-simplified 銜接依 §8-D6 的雙工具鏈精神，
  改採 shell 條件式（`tools/scan-simplified.ts` 存在才執行）以承接 M0-6（掃描器本體）
  與 M0-7（本任務）可能不同完成順序的並行情境，避免任一方先完成時另一方紅燈。
  ——後記（同日 M0 checkpoint）：M0-6/M0-7 同批完成後，條件式已無必要，`validate:data`
  定案為三段直串（zod＋掃描器＋字型涵蓋率），以 `package.json` 為最終真相（01 §8-D15）。
- **D17｜M0 checkpoint 之雲端驗證項延後（2026-07-11）**：§3.3.2 DoD 第 5 條（PR 觸發 CI
  五 job、merge 後 Pages URL 200）與 §3.14 第 4 步 tag 推送需要 push 至 GitHub（origin），
  屬對外動作，依 orchestrator 之保守原則**待使用者核准後執行**。本地可驗項（DoD 1–4、
  §3.14 第 1–3 步）已全數完成並活體驗證；tag `m0` 先行建立於本地、待核准後與 main 一併推送。
  ——已完成（同日）：使用者核准後推送 main＋tag m0；CI 五 job（install→lint/typecheck/unit/e2e）全綠；
  Pages 以 build_type=workflow 啟用（首次 deploy 因 Pages 尚未啟用而失敗、re-run 成功），
  `https://tigercosmos.github.io/tenka-fubu/` 回 200 且 title=天下布武。M0 DoD 六條全數滿足。
- **D18｜M4 手動戰爭迴圈 setup（2026-07-13，M4 fix-forward）**：M7 AI 尚未落地，且 s1560 初始
  `armies` 為空；原 §3.7 DoD 無法由未改動劇本的單方 UI 操作自然產生齋藤迎擊軍與「先制壓一郡」路徑。
  故以 `?debug=1` 建立決定論 setup，但部隊仍須由正式 Command 出陣，setup 完成後的制壓、野戰、圍城、
  落城與報告驗證均走正式 daily tick，不以直接改寫 state 代替系統流程。
- **D19｜M6 插入阻斷 `M6-V` 視覺基礎工作串流（2026-07-14，優先級調整）**：M0–M5 已完成、
  `milestone.json.current` 已為 M6，故不把 M2–M5 回改成未完成，也不新增會破壞既有 milestone enum／CI
  前綴規則的獨立里程碑。現況 audit 顯示地圖已有 8 層骨架、`TerritoryGrid`、Castle／District scenePart、
  ArmyChip 與多個 HUD 元件，但 production 仍停在單色底圖、幾何節點、簡單直線道路與未組裝 UI；問題是
  跨層整合與素材生產缺失，而非玩法 core 從零重寫。定案在 M6 內用 `M6-V1`～`M6-V11` 作全域開工 gate，
  依「原創規範／固定 fixture → 素材與 view contract → 地形 → 道路 → 城池 → 軍隊 → HUD → 分析層 →
  品質 gate」交付。參考《信長之野望・新生》僅萃取地圖資訊層級與操作回饋，禁止複製任何具體美術資產；
  素材來源與授權由 manifest 阻斷。技術與視覺細節仍回歸 04／11／12／17，本文件只裁定優先序與 DoD。
