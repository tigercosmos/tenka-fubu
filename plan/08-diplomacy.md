...
| 文件 | 關係 |
|---|---|
| `plan/02-data-model.md` | 本文件的 interface 由 02 彙整為最終型別；`Officer.isKin`（一門）、`Officer.spouseId`、`Castle.betrayalReadyClanId` 等跨域欄位以 02 定案。 |
...
所有行動由 `diplomacy/propose` Command 發起，產生 `DiplomacyProposal` 送達對方；
...
| 婚姻同盟 | 已有生效中同盟；雙方各有至少一名「適齡一門」（`isKin`、年齡 `BAL.marriageMinAge=13`..`BAL.marriageMaxAge=45`、`spouseId===null`） | `BAL.trustCostMarriage = 40` | `BAL.goldCostMarriage = 500` | 附著於同盟 | 同盟到期日延長至「現在＋60 月」與原到期日之較大者；設定 `noBreakUntilDay = 現在 + BAL.marriageNoBreakMonths(=36) 月`；兩名一門武將互設 `spouseId` |
...
- 各勢力對朝廷有友好度 `imperialFavor`：0..`BAL.courtFavorMax = 100`。
...
敘任規則：以 `diplomacy/requestCourtRank` Command 申請；必須逐階敘任（下一階＝現階＋1）；
條件＝友好度達標＋一次性獻金（金錢）；敘任即**永久取得**（不會失去；當主更替不影響，官位屬勢力名義）；
威信加成於敘任時一次性加算至勢力威信；解鎖效果持續有效。僅最高官位的名稱顯示於 UI。

| 階 | id | 官位 | 友好度需求 | 獻金（貫） | 威信加成 | 解鎖效果 |
|---|---|---|---|---|---|---|
| 1 | `crank.ju5ge` | 從五位下 | 20 | 300 | +50 | — |
| 2 | `crank.ju5jo` | 從五位上 | 30 | 500 | +80 | 外交工作信用增益 ×1.1（`trustGainBonus`） |
| 3 | `crank.sho5ge` | 正五位下 | 40 | 800 | +120 | 解鎖**停戰斡旋**（`mediation`） |
| 4 | `crank.ju4ge` | 從四位下 | 50 | 1200 | +170 | **政策槽 +1**（`policySlot`，參見 05） |
| 5 | `crank.ju4jo` | 從四位上 | 60 | 1800 | +230 | 從屬勸告接受度 +10（`vassalDemandBonus`） |
| 6 | `crank.sho4ge` | 正四位下 | 70 | 2500 | +300 | 停戰斡旋友好度成本 ×0.75（`mediationDiscount`） |
| 7 | `crank.ju3` | 從三位 | 85 | 3500 | +400 | 同盟／不可侵接受度 +5（`allianceBonus`） |
| 8 | `crank.sho1` | 正一位 | 95 | 5000 | +550 | **政策槽再 +1**（`policySlot`，與第 4 階累計共 +2） |

上表以常數 `BAL.courtRankTable`（`CourtRankDef[]`）實作於 `balance.ts`。

#### 3.5.3 停戰斡旋

- 條件：我方官位 ≥ 第 3 階（正五位下）；與目標 atWar；我方對「斡旋」的冷卻已過
...
`BAL.shogunateFavorDecayMonthly = 1`）。役職以 `diplomacy/requestShogunateTitle` 申請，
逐階敘任、威信一次性加算（但幕府滅亡時**收回**，見 §3.6.4）、效果持有期間有效。

| 階 | id | 役職 | 條件 | 獻金（貫） | 威信加成 | 效果 |
|---|---|---|---|---|---|---|
| 1 | `stitle.otomoshu` | 御供眾 | 威信 ≥ 150、幕府友好度 ≥ 30 | 500 | +60 | 幕府相關歷史事件解鎖額外選項（參見 10） |
| 2 | `stitle.oshobanshu` | 御相伴眾 | 威信 ≥ 350、友好度 ≥ 50、任御供眾 | 1000 | +120 | 外交工作與獻金工作每月費用 ×0.8 |
| 3 | `stitle.kanreidai` | 管領代 | 威信 ≥ 600、友好度 ≥ 70、任御相伴眾 | 2000 | +200 | 從屬勸告接受度 +15 |
| 4 | `stitle.kanrei` | 管領 | 威信 ≥ 900、友好度 ≥ 85、任管領代、**控制京都所在城或為擁立者** | 3500 | +300 | **政策槽 +1**（參見 05） |
| 5 | `stitle.fukushogun` | 副將軍 | 威信 ≥ 1200、友好度 ≥ 90、**必須為擁立將軍者**、任管領 | 5000 | +400 | 大義名分：我方所有外交行動接受度 +10 |

上表以常數 `BAL.shogunateTitleTable`（`ShogunateTitleDef[]`）實作。

#### 3.6.3 上洛與將軍擁立

- 「京都所在城」由劇本資料欄位 `scenario.court.capitalCastleId` 指定（s1560 ＝ 二條御所所在之
  `castle.nijo`，資料細節參見 14）。
- 擁立條件（`diplomacy/nominateShogun`）：`shogunate.exists`、`patronClanId === null`（全域唯一）、
...
- 以 `diplomacy/startPlot` 下達；執行武將條件同外交工作（未出陣、一人一件）。
...
- 可隨時 `diplomacy/cancelPlot` 中止（無退款、不判定敗露）。

#### 3.7.1 引拔（poach）——挖角敵方武將

| 項目 | 規格 |
|---|---|
| 目標條件 | 敵方武將：`loyalty < BAL.plotPoachLoyaltyThreshold = 75`、非當主、非一門（`isKin === false`）、未被俘 |
...
| 成功效果 | 該城標記 `betrayalReadyClanId = 我方`、`betrayalReadyUntilDay = 今日 + BAL.plotBetrayalMarkMonths(=12)×30`；我軍圍攻該城期間可下達 `diplomacy/activateBetrayal` **發動**：城士氣立即降至 `BAL.plotBetrayalMoraleFloor = 5`、該城主忠誠歸 0（攻城與戰後捕虜處理參見 07／06）；發動後標記清除 |
...
export interface CourtState {
  imperialFavor: Record<ClanId, number>;         // 朝廷友好度 0..100（點）
...
export type ShogunateTitleId =
  | 'stitle.otomoshu' | 'stitle.oshobanshu' | 'stitle.kanreidai'
  | 'stitle.kanrei' | 'stitle.fukushogun';
...
export interface ShogunateState {
  ...
  grantedTitle: Record<ClanId, ShogunateTitleId | null>; // 已敘任最高役職
}
...
export type DiplomacyCommand =
  | { type: 'diplomacy/assignWork'; clanId: ClanId; officerId: OfficerId; target: DiplomacyWorkTarget }
  | { type: 'diplomacy/cancelWork'; clanId: ClanId; officerId: OfficerId }
  | { type: 'diplomacy/propose'; clanId: ClanId; kind: DiplomacyActionKind; targetClanId: ClanId;
      reinforceAgainstClanId?: ClanId; marriageOfficerId?: OfficerId /* 我方一門，婚姻用 */ }
  | { type: 'diplomacy/respond'; clanId: ClanId; proposalId: ProposalId; accept: boolean }
  | { type: 'diplomacy/breakPact'; clanId: ClanId; pactId: PactId }
  | { type: 'diplomacy/requestCourtRank'; clanId: ClanId }
  | { type: 'diplomacy/requestCourtMediation'; clanId: ClanId; targetClanId: ClanId }
  | { type: 'diplomacy/requestShogunateTitle'; clanId: ClanId }
  | { type: 'diplomacy/nominateShogun'; clanId: ClanId }
  | { type: 'diplomacy/startPlot'; clanId: ClanId; kind: PlotKind; officerId: OfficerId;
      targetClanId: ClanId; targetOfficerId?: OfficerId; targetCastleId?: CastleId; investGold: number }
  | { type: 'diplomacy/cancelPlot'; clanId: ClanId; plotId: PlotId }
  | { type: 'diplomacy/activateBetrayal'; clanId: ClanId; castleId: CastleId };
```

跨域欄位（於 02 定案，責任歸屬如下）：

- `Officer.isKin: boolean`（一門）、`Officer.spouseId: OfficerId | null` —— 婚姻同盟使用（06 定義一門本體）。
...
2. `trust ∈ [0,100]`、`sentiment ∈ [-100,100]`、`imperialFavor/shogunateFavor ∈ [0,100]`、`progress ∈ [0,100]`。
...
`dip.pactSigned`、`dip.pactExpired`、`dip.pactBroken`、`dip.proposalReceived`、
...
diplomacyDailyTick(state):
  # 每日
  1. 協定到期：for pact in pacts（依 id 序）: if day > pact.expiryDay → 移除（雙側 pactIds 同步）、emit dip.pactExpired
...
diplomacyMonthlyTick(state):
  ...
  for clan in clans（依 clanId 字典序，確保決定論）:
    1. 工作扣費：每件 work 依目標扣月費（含御相伴眾 ×0.8）；金錢不足 → 中止該件、emit dip.workStopped
...
#### 5.3.1 提案建立（`diplomacy/propose` 驗證器）
...
resolveProposal(state, prop, accept):
  ...
  扣 from 方信用與金錢 → 依種類套用效果：
    ...
    requestReinforce     → rows[from][to].lastReinforceRequestDay = day；
                           派兵決策與部隊生成移交 09/07
  套用一次性感情（§3.3 表）；emit dip.pactSigned / dip.proposalResolved
```

#### 5.3.3 破棄（`diplomacy/breakPact`）
...
#### 5.4.1 官位敘任（`diplomacy/requestCourtRank`）

```
next = courtRankByClan[clan] + 1；next ≤ 8
def  = BAL.courtRankTable[next-1]
驗證: imperialFavor[clan] ≥ def.favorRequired 且 clan.gold ≥ def.goldCost
...
執行: 扣 favorCost 與金錢
      ...
      if rng.misc() < failP: gold += 250（退還半額）；emit court.mediationResult(失敗)
            ...
            emit court.mediationResult(成功) + dip.pactSigned
...
#### 5.5.3 內應發動（`diplomacy/activateBetrayal`）
...
| `BAL.plotPoachLoyaltyThreshold/ProgressBase/ProgressIntDivisor/Base/LoyaltyDivisor/IntDivisor/GoldDivisor/PMin/PMax/InitialLoyalty` | 75 / 25 / 4 / 0.10 / 200 / 300 / 2500 / 0.05 / 0.85 / 60 | 引拔 |
