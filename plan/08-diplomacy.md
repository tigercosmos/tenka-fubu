...
「交戰中」為衍生狀態，不另設宣戰指令：
`atWar(A,B) := getRow(A,B).lastHostileDay 在 BAL.warStateMonths（建議 6）個月內`，
且雙方無生效中的停戰／同盟／婚姻／從屬。`lastHostileDay` 由 07 在野戰、合戰、攻城、
制壓敵郡發生時回寫至該對列（無向、單一值）。
...
本節僅列與**本系統機制語意**相關之約定，不重複型別宣告（避免與 02 漂移）；朝廷／幕府之
`CourtState`／`ShogunateState`／平衡表列型別仍於本節列出（其效果與敘任規則由本文件擁有）。
...
（02 §5.5）。方向存取語糖 `trust(X→Y)`／`sentiment(X→Y)` 見 §3.1。此外本系統於該對列讀寫三個
**每對機制狀態欄位**（語意由本文件擁有）：`lastHostileDay`（atWar 推導，§3.1）、
`refusalCooldownUntilDay: Partial<Record<DiplomacyActionKind, number>>`（提案拒絕冷卻，§5.3.2）、
`lastReinforceRequestDay`（援軍請求冷卻，§5.3.5）。
...
`target: ClanId | 'court'`——對勢力累積信用、`'court'` 累積朝廷友好度（獻金工作，勘誤 E-27）。
幕府友好度之累積機制見 §3.6.2（`target='shogunate'` 之型別歸屬見 §8 記錄）。
...
//   CmdStartDiploWork { target: ClanId|'court'; executorId }        // 執行武將以 executorId 傳入（勘誤 E-27／二輪裁決 A）
//   CmdStopDiploWork  { target: ClanId|'court' }                    // 僅 target（不帶 officerId，勘誤 E-28）
//   CmdProposePact    { targetClanId; kind: PactKind; termDays }    // 提案協定
...
```
validatePropose(state, cmd):   # cmd = CmdProposePact{ targetClanId, kind, termDays }
...
resolveProposal(state, prop, accept):
  ...
  扣 from 方信用（trust(from→to) -= 成本）與金錢 → 依種類建立內嵌 Pact（皆 startDay=day）：
    ...
    requestReinforce     → row.lastReinforceRequestDay = day；
                           派兵決策與部隊生成移交 09/07（不建立 Pact）
