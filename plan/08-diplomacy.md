...
- `trustAtoB`＝a 對 b 累積的信用、`trustBtoA`＝b 對 a 累積的信用（各 0..100，**允許小數累積、顯示時 floor**；發起方可花費它對對方提案；與 `sentiment`〔E-24〕及 `courtFavor`/`shogunateFavor` 同為小數內部儲存機制，02 D16）。
...
接壤判定由 **tick-transient 快取** `adjacencyCache: Set<string /* pairKey 字典序 */>` 承載
（不序列化、不入狀態雜湊，02 DDR-8）：於**每個 tick 內首次需要接壤判定時** lazy 重算一次
（掃描全部 RoadEdge 兩端歸屬，參見 04），於該 tick 內快取重用、tick 結束即丟棄——
**非**「每月 1 日重算」（月中發生的合戰威風〔§5.2.4〕等於任意日觸發者，讀取的皆為當日最新接壤關係）。
...
對方為 AI 時於**同一 tick 的 diplomacy 步驟（00 §5.4 第 4 步；受方提案之唯一結算點，與 03／09 一致）**
以 §5.6 `evaluateProposal` 決定論即時判定；對方為玩家時觸發「外交來使」自動暫停（00 §5.2），
提案存活 `BAL.dipProposalExpiryDays`（建議 30）日，逾期視同拒絕。
...
diplomacyMonthlyTick(state):
  # adjacencyCache 不在此顯式重算：接壤判定於本 tick 首次需要時 lazy 重算並於 tick 內快取（§3.1）
  # ── 每勢力項（per-clan）──
  for clan in clans（依 clanId 字典序，確保決定論）:
    ...
    4. 朝廷/幕府友好度衰減（當月無對應獻金工作時 −BAL.courtFavorDecayMonthly / shogunateFavorDecayMonthly）
    5. 擁立者資格檢查（§3.6.3）；擁立者每月威信 +BAL.shogunPatronMonthlyPrestige
  # ── 每對感情項（per-pair）：改「for row in rows 依 key 字典序、每對僅一次」，
  #    消除原 for-clan 迴圈對同一 pair（clan=a 與 clan=b）各套用一次的雙倍問題 ──
  for row in rows（依 key 字典序，確保決定論）:
    6. 感情月變動（§5.2.3：交戰/接壤緊張/共同敵人；三因素對稱、雙向同增量一次套用）
    7. 感情漂移向 0（BAL.sentimentDriftMonthly）並 clamp
  # 接壤緊張／共同敵人若涉及尚未物化之 pair，於套用時經 getRow 依需要 materialize（§3.1 稀疏列），確保無 pair 漏算
```
...
增益值**以小數累積於 `trust*`（不逐月捨入），僅顯示時 floor**（02 D16、對齊 `sentiment` E-24）；
故上例政務 100、感情 0 之武將 10 個月恰累積 60（6.0/月×10，誤差 0，決定論；驗收見 T2）。
...
`courtFavor`／`shogunateFavor` 亦**以小數累積、顯示時 floor**（同 §5.2.1 trust，02 D16）；衰減（§5.1 步驟 4）同以小數扣減。
...
resolveProposal(state, prop, accept):
  ...
  扣 from 方信用（trust(from→to) -= 成本）與金錢 → 依種類建立內嵌 Pact（皆 startDay=day）：
    ...
    requestReinforce     → 設 lastReinforceRequestDay(from→to) = day（有向存取語糖，§4／§5.3.5）；
                           ...
                           emit diplo.reinforceAgreed{ fromClanId: from, toClanId: to, againstClanId: prop.reinforceAgainstClanId }；
                           return（**不套用簽約感情、不 emit pact.signed**——未締約，pact.signed payload 無 Pact 可承載）
  # 以下僅締約種類（alliance/ceasefire/marriage/vassal）執行：
...
- [ ] **T1 資料結構與初始化**：實作 §4 全部型別；劇本載入時**僅為劇本明列非預設關係之 pair** materialize
      `DiplomacyRow`（trust=0、sentiment 依劇本資料、協定內嵌 `pacts`；未列之 pair 為稀疏缺列，
      經 `getRow` 回預設列＝雙向 trust=0／sentiment=0／無協定，§3.1／02 §5.5 `defaultRow`），
      `CourtState`（s1560：全勢力 `courtFavor`/`shogunateFavor`=0、`Clan.courtRank`/`Clan.shogunateTitle`='none'、
      幕府 exists=true、義輝在世、patron=null）。
      驗收：載入 s1560 後未列 pair 之 `getRow` 回預設列、已列 pair 值正確；**無「全對」物化、無有向鏡像複製**
      （E-28 已廢有向 `byClan`／`pactIds` 鏡像模型，稀疏列為單一真相）。
...
**（2026-07-11）驗證修復（六員對抗式驗證 CONFIRMED findings，02 四輪裁決連動）**：
- **感情月結雙倍套用（E23）**：§5.1 `diplomacyMonthlyTick` 原將「感情月變動／漂移」置於 `for clan in clans`
  迴圈內，致同一 pair（clan=a 與 clan=b）各套用一次（雙倍）。改拆為兩段：per-clan（工作扣費/結算、調略、
  友好度衰減、擁立者檢查）與 **per-pair `for row in rows` 依 key 字典序、每對僅一次**（感情月變動＋漂移）；
  接壤緊張／共同敵人涉及未物化 pair 者經 `getRow` 依需要 materialize，確保無 pair 漏算。
- **adjacencyCache lazy 重算（E24）**：§3.1／§5.1 原述「每月 1 日於系統開頭重算」與其 tick-transient 定性矛盾
  （月中觸發之合戰威風讀不到當日接壤）。改為**每 tick 內首次需要接壤判定時 lazy 重算、tick 內快取、tick 結束丟棄**，
  刪 §5.1 顯式「步驟 0 重算」。
- **requestReinforce 接受分支誤發 `pact.signed`（E21）**：§5.3.2 該分支不建立 Pact，原落入通用
  「套用簽約感情；emit pact.signed」尾段（payload 無 Pact 可承載）。改為發 **`diplo.reinforceAgreed{fromClanId,
  toClanId,againstClanId}`** 後 return，不套簽約感情、不 emit `pact.signed`；§4 事件清單新增該 08 擴充事件
  （13 §6.11 對應報告、派兵決策 09/07）。
- **T1 驗收改稀疏語意（finding 4）**：原「建立全對 `DiplomacyRow`／rows 完整成對／鏡像不變量」與 E-28 稀疏無向列
  模型矛盾。改為僅劇本明列 pair materialize、其餘缺列＝`getRow` 預設列（02 §5.5 `defaultRow`）；官位/役職初始
  `Clan.courtRank`/`Clan.shogunateTitle`='none'、友好度 `courtFavor`/`shogunateFavor`=0；驗收改「無全對物化、無有向鏡像」。
- **trust／friendship 小數累積（02 D16）**：§3.1 trust 欄位、§5.2.1／§5.2.2 補明 `trust*`／`courtFavor`／
  `shogunateFavor` **以小數內部儲存、顯示時 floor**（對齊 `sentiment` E-24）；與 T2「10 月恰 60」相容（6.0/月×10）。
- **AI 受方提案結算點統一措辭（E22）**：§3.4 導言、§5.3.1 明訂受方提案之**唯一結算點＝00 §5.4 第 4 步 diplomacy**
