...
| `court.mediationResult` | `payload.success`（true/false） | `report.court.mediationSuccess`／`report.court.mediationFailed` | success：`clan`←`targetClanId`, `months`←`ceasefireMonths`（number）；!success：`clan`←`targetClanId`（六輪裁決 1；成功時同 tick 另有 `pact.signed`→`report.diplomacy.pactSigned`，兩則並存不去重，見 §8） |
...
| `transport.looted` | `byClanId` 與 `playerClanId`（`e.clanIds` 判別輸送隊歸屬）：`byClanId≠playerClanId` 且輸送隊屬玩家（被劫方＝我方）→**looted**；`byClanId=playerClanId`（劫方＝我方）→**lootGain**；皆非→**null** | `report.transport.looted`／`report.transport.lootGain`／**null** | looted：`place`←`nodeId`, `clan`←`byClanId`；lootGain：`place`←`nodeId`（六輪裁決追記） |
...
5. §3.7 契約：對每個會產生報告的 GameEventType 構造假事件＋最小 state → `deriveReportMessage`
   ...
   以 won/lost/resolved／subjugated/districtLost／exposed/exposedByEnemy／looted/lootGain 各視角各構造一例
   （`battle.ended` 另構造 `winnerClanId=null` 平手一例斷言走 resolved 而非 lost；`transport.looted` 另構造
   `byClanId`／`e.clanIds` 皆非玩家之第三方一例斷言回傳 null）；payload 分岔事件（`uprising.ended` suppressed/subsided、
   `court.mediationResult` success/!success）各分支各構造一例；視角過濾事件（`shogunate.patronLost`）
   構造當事勢力與非當事勢力各一例、後者斷言回傳 null；`diplo.envoyArrived` 斷言 `{proposal}` 解自
   `payload.kind`（非 proposalId）；排除清單事件（§3.7(5)）斷言回傳 null（`save.autosaveFailed` 非 GameEventType、
   不在本項母集，六輪裁決 4）
```
...
| `report.court.mediationSuccess` | `court.mediationResult`(success) | clan, months | 朝廷斡旋成功，與{clan}停戰{months}月。 |
| `report.court.mediationFailed` | `court.mediationResult`(!success) | clan | 朝廷斡旋失敗，{clan}官位在我方之上。 |
