...
5. §3.7 契約：對每個會產生報告的 GameEventType 構造假事件＋最小 state → `deriveReportMessage`
   ...
   五輪裁決 A/B）；玩家視角分流事件（`battle.ended`／`district.subjugated`／`plot.exposed`）以 won/lost/resolved
   ／subjugated/districtLost／exposed/exposedByEnemy 各視角各構造一例（`battle.ended` 另構造 `winnerClanId=null`
   平手一例斷言走 resolved 而非 lost）；payload 分岔事件（`uprising.ended` suppressed/subsided、
