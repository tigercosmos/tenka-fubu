...
describe('recolorTerritory（04 §5.5：pass1 勢力索引／pass2 base 色＋界線烘焙）', () => {
  ...
  it('owner cell → clanColorNum(colorIndex)，alpha=255；無主 → MAPVIEW.colors.neutral', () => {
  ...
  it('兩勢力邊界：交界兩側 cell 皆乘 borderDarken，非邊界 cell 維持原色', () => {
    ...
    const expectedLeft = {
      r: Math.round(((oda >> 16) & 0xff) * darken),
      g: Math.round(((oda >> 8) & 0xff) * darken),
      b: Math.round((oda & 0xff) * darken),
      a: 255,
    };
    expect(left).toEqual(expectedLeft);
    expect(right).not.toEqual(expectedLeft); // 另一側是 imagawa 色（已各自變暗）
